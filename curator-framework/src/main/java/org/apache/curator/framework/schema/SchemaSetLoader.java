...
/**
 * <p>
 * Utility to load schems set from a JSON stream/file. <strong>NOTE:</strong> to
 * avoid adding a new dependency to Curator, the Jackson library has been used
 * with "provided" scope. You will need to add a dependency to
 * <code>com.fasterxml.jackson.core:jackson-core:2.7.3</code> and
 * <code>com.fasterxml.jackson.core:jackson-databind:2.7.3</code> to your
 * project
 * </p>
 *
 * <p>
 * The JSON stream should be an array of named schemas:<br>
 * <br>
 * <code><pre>
 * [
 *     {
...
public class SchemaSetLoader {
	...
	/**
	 * Called to map a schema validator name in the JSON stream to an actual data
	 * validator
	 */
	 ...
	 * @param schemaValidatorMapper mapper from validator name to instance - can be
	 *                              null if not needed
	 */
	...
	/**
	 * @param useDefaultSchema if true, return a default schema when there is no
	...
	private void read(ImmutableList.Builder<Schema> builder, JsonNode node,
	...
	private void readNode(ImmutableList.Builder<Schema> builder, JsonNode node,
			SchemaValidatorMapper schemaValidatorMapper) {
		...
		Schema schema = schemaBuilder.name(name).documentation(getText(node, "documentation", ""))
				.ephemeral(getAllowance(node, "ephemeral")).sequential(getAllowance(node, "sequential"))
				.watched(getAllowance(node, "watched")).canBeDeleted(getBoolean(node, "canBeDeleted"))
	...
	private Schema.Allowance getAllowance(JsonNode node, String name) {
		JsonNode namedNode = node.get(name);
		try {
			return (namedNode != null) ? Schema.Allowance.valueOf(namedNode.asText().toUpperCase())
