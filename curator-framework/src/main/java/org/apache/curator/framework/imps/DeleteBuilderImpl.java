...
public class DeleteBuilderImpl implements DeleteBuilder, BackgroundOperation<String>, ErrorListenerPathable<Void> {
	...
	public DeleteBuilderImpl(CuratorFrameworkImpl client, int version, Backgrounding backgrounding,
			boolean deletingChildrenIfNeeded, boolean guaranteed, boolean quietly) {
		this.client = client;
	...
	<T> TransactionDeleteBuilder<T> asTransactionDeleteBuilder(final T context,
			...
			client.getZooKeeper().delete(operationAndData.getData(), version, new AsyncCallback.VoidCallback() {
				...
				public void processResult(int rc, String path, Object ctx) {
					...
					if ((rc == KeeperException.Code.NOTEMPTY.intValue()) && deletingChildrenIfNeeded) {
						...
						CuratorEvent event = new CuratorEventImpl(client, CuratorEventType.DELETE, rc, path, null, ctx,
	...
	public Void forPath(String path) throws Exception {
		...
		if (backgrounding.inBackground()) {
			...
			client.processBackgroundOperation(new OperationAndData<String>(this, path, backgrounding.getCallback(),
					errorCallback, backgrounding.getContext(), null), null);
		} else {
