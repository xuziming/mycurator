...
public class ConnectionStateManager implements Closeable {
	 ...
	 * @param sessionExpirationPercent percentage of negotiated session timeout to
	 *                                 use when simulating a session timeout. 0
	 *                                 means don't simulate at all
	 */
	public ConnectionStateManager(CuratorFramework client, ThreadFactory threadFactory, int sessionTimeoutMs,
		...
		this(client, threadFactory, sessionTimeoutMs, sessionExpirationPercent,
				ConnectionStateListenerDecorator.standard);
	}
	 ...
	 *                                         default
	 * @param sessionTimeoutMs                 the ZK session timeout in
	 *                                         milliseconds
	 * @param sessionExpirationPercent         percentage of negotiated session
	 *                                         timeout to use when simulating a
	 *                                         session timeout. 0 means don't
	 *                                         simulate at all
	...
	public ConnectionStateManager(CuratorFramework client, ThreadFactory threadFactory, int sessionTimeoutMs,
		...
		listeners = StandardListenerManager.mappingStandard(listener -> listener.doNotDecorate() ? listener
	...
	/**
	 * Change to {@link ConnectionState#SUSPENDED} only if not already suspended and
	 * not lost
	 *
	...
	/**
	 * Post a state change. If the manager is already in that state the change is
	 * ignored. Otherwise the change is queued for listeners.
	 *
	 * @param newConnectionState new state
	 * @return true if the state actually changed, false if it was already at that
	 *         state
	 */
	...
	private void processEvents() {
		while (state.get() == State.STARTED) {
			try {
				...
				long elapsedMs = startOfSuspendedEpoch == 0 ? useSessionTimeoutMs / 2
	...
	private void checkSessionExpiration() {
		if ((currentConnectionState == ConnectionState.SUSPENDED) && (startOfSuspendedEpoch != 0)) {
			...
			if (elapsedMs >= useSessionTimeoutMs) {
				startOfSuspendedEpoch = System.currentTimeMillis(); // reset startOfSuspendedEpoch to avoid spinning on
																	// this session expiration injection CURATOR-405
			...
			try {
				// give ConnectionState.checkTimeouts() a chance to run, reset ensemble
				// providers, etc.
