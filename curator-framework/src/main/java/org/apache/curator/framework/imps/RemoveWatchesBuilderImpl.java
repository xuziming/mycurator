...
public class RemoveWatchesBuilderImpl implements RemoveWatchesBuilder, RemoveWatchesType, RemoveWatchesLocal,
	...
	public RemoveWatchesBuilderImpl(CuratorFrameworkImpl client, Watcher watcher, CuratorWatcher curatorWatcher,
			WatcherType watcherType, boolean guaranteed, boolean local, boolean quietly, Backgrounding backgrounding) {
		this.client = client;
	...
	private void pathInForeground(final String path) throws Exception {
		...
		// For the local case we don't want to use the normal retry loop and we don't
		// want to block until a connection is available.
		// We just execute the removeWatch, and if it fails, ZK will just remove local
		// watches.
		if (local) {
				...
				public Void call() throws Exception {
					try {
						...
						if (RetryLoop.isRetryException(e) && guaranteed) {
							...
							client.getFailedRemoveWatcherManager().addFailedOperation(
									new FailedRemoveWatchManager.FailedRemoveWatchDetails(path, finalNamespaceWatcher));
							throw e;
		...
		try {
			...
			AsyncCallback.VoidCallback callback = new AsyncCallback.VoidCallback() {
				...
				public void processResult(int rc, String path, Object ctx) {
					trace.commit();
					CuratorEvent event = new CuratorEventImpl(client, CuratorEventType.REMOVE_WATCHES, rc, path, null,
