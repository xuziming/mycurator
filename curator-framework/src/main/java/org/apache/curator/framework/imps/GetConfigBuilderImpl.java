...
public class GetConfigBuilderImpl
	...
	public byte[] forEnsemble() throws Exception {
		if (backgrounding.inBackground()) {
			client.processBackgroundOperation(new OperationAndData<Void>(this, null, backgrounding.getCallback(), null,
					backgrounding.getContext(), watching), null);
			return null;
		} else {
		...
		try {
			...
			AsyncCallback.DataCallback callback = new AsyncCallback.DataCallback() {
				...
				public void processResult(int rc, String path, Object ctx, byte[] data, Stat stat) {
					...
					trace.commit();
					CuratorEvent event = new CuratorEventImpl(client, CuratorEventType.GET_CONFIG, rc, path, null, ctx,
