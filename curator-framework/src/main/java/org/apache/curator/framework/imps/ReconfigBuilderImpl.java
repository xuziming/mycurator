...
public class ReconfigBuilderImpl implements ReconfigBuilder, BackgroundOperation<Void>, ErrorListenerReconfigBuilderMain {
	...
	public ReconfigBuilderImpl(CuratorFrameworkImpl client, Backgrounding backgrounding, Stat responseStat,
			long fromConfig, List<String> newMembers, List<String> joining, List<String> leaving) {
		this.client = client;
	...
	public byte[] forEnsemble() throws Exception {
		if (backgrounding.inBackground()) {
			client.processBackgroundOperation(new OperationAndData<>(this, null, backgrounding.getCallback(), null,
		...
		try {
			...
			AsyncCallback.DataCallback callback = new AsyncCallback.DataCallback() {
				...
				public void processResult(int rc, String path, Object ctx, byte[] bytes, Stat stat) {
					...
					CuratorEvent event = new CuratorEventImpl(client, CuratorEventType.RECONFIG, rc, path, null, ctx,
							stat, bytes, null, null, null, null);
