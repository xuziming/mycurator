...
public class GetDataBuilderImpl implements GetDataBuilder, BackgroundOperation<String>, ErrorListenerPathable<byte[]> {
	...
	public GetDataBuilderImpl(CuratorFrameworkImpl client, Stat responseStat, Watcher watcher,
			Backgrounding backgrounding, boolean decompress) {
		this.client = client;
			...
			AsyncCallback.DataCallback callback = new AsyncCallback.DataCallback() {
				...
				public void processResult(int rc, String path, Object ctx, byte[] data, Stat stat) {
					...
					trace.setReturnCode(rc).setResponseBytesLength(data).setPath(path)
							.setWithWatcher(watching.hasWatcher()).setStat(stat).commit();
	...
	private byte[] pathInForeground(final String path) throws Exception {
		...
		trace.setResponseBytesLength(responseData).setPath(path).setWithWatcher(watching.hasWatcher())
				.setStat(responseStat).commit();
