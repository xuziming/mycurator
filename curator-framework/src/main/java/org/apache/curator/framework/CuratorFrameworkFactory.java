...
public class CuratorFrameworkFactory {
	...
	public static class Builder {
		 ...
		 * Temporary CuratorFramework instances are meant for single requests to
		 * ZooKeeper ensembles over a failure prone network such as a WAN. The APIs
		 * available from {@link CuratorTempFramework} are limited. Further, the
		 * connection will be closed after 3 minutes of inactivity.
		 *
		 ...
		 * ZooKeeper ensembles over a failure prone network such as a WAN. The APIs
		 * available from {@link CuratorTempFramework} are limited. Further, the
		 * connection will be closed after <code>inactiveThresholdMs</code> milliseconds
		 * of inactivity.
		 *
		 * @param inactiveThreshold number of milliseconds of inactivity to cause
		 *                          connection close
		...
		/**
		 * Add connection authorization. The supplied authInfos are appended to those
		 * added via call to {@link #authorization(java.lang.String, byte[])} for
		...
		/**
		 * Set the list ensemble provider. IMPORTANT: use either this or
		 * {@link #connectString(String)} but not both.
		 *
		 ...
		 * This is useful for debugging purposes. For example, you could set this to be
		 ...
		 * pre-defined namespace. If a namespace is set here, all paths will get
		 * pre-pended with the namespace
		 *
		 ...
		 *                      in case of a network partition. See
		 ...
		 * {@link CreateBuilder#creatingParentContainersIfNeeded()} if the ZK JAR
		 * supports {@link CreateMode#CONTAINER}. Call this method to turn off this
		 * behavior.
		 *
		 * @return this
		 */
		...
		/**
		 * Set the error policy to use. The default is
		 * {@link StandardConnectionStateErrorPolicy}
		 *
		 * @since 3.0.0
		...
		/**
		 * If mode is true, create a ZooKeeper 3.4.x compatible client. IMPORTANT: If
		 * the client library used is ZooKeeper 3.4.x <code>zk34CompatibilityMode</code>
		...
		/**
		 * Set a timeout for {@link CuratorZookeeperClient#close(int)} }. The default is
		 * 0, which means that this feature is disabled.
		 *
		 * @since 4.0.2
		 ...
		 * </p>
		 * <p>
		 * <strong>IMPORTANT: </strong> StandardConnectionHandlingPolicy has different
		 * behavior than the connection policy handling prior to version 3.0.0.
		 * </p>
		 * <p>
		 * Major differences from the older behavior are:
		 * </p>
		 * <ul>
		 * <li>Session/connection timeouts are no longer managed by the low-level
		 * client. They are managed by the CuratorFramework instance. There should be no
		 * noticeable differences.</li>
		 * <li>Prior to 3.0.0, each iteration of the retry policy would allow the
		 * connection timeout to elapse if the connection hadn't yet succeeded. This
		 * meant that the true connection timeout was the configured value times the
		 * maximum retries in the retry policy. This longstanding issue has been
		 * address. Now, the connection timeout can elapse only once for a single API
		 * call.</li>
		 * <li><strong>MOST IMPORTANTLY!</strong> Prior to 3.0.0,
		 * {@link ConnectionState#LOST} did not imply a lost session (much to the
		 * confusion of users). Now, Curator will set the LOST state only when it
		 * believes that the ZooKeeper session has expired. ZooKeeper connections have a
		 * session. When the session expires, clients must take appropriate action. In
		 * Curator, this is complicated by the fact that Curator internally manages the
		 * ZooKeeper connection. Now, Curator will set the LOST state when any of the
		 * following occurs: a) ZooKeeper returns a
		 * {@link Watcher.Event.KeeperState#Expired} or
		 * {@link KeeperException.Code#SESSIONEXPIRED}; b) Curator closes the internally
		 * managed ZooKeeper instance; c) The session timeout elapses during a network
		 * partition.</li>
		 * </ul>
		 *
		...
		/**
		 * Curator (and user) recipes will use this executor to call notifyAll and other
		 * blocking calls that might normally block ZooKeeper's event thread. By
		 * default, an executor is allocated internally using the provided (or default)
		 * {@link #threadFactory(java.util.concurrent.ThreadFactory)}. Use this method
		 * to set a custom executor.
		 *
		 * @param runSafeService executor to use for calls to notifyAll from Watcher
		 *                       callbacks etc
		 * @return this
		 * @since 4.1.0
		 */
		...
		/**
		 * Sets the connection state listener decorator. For example, you can set
		 * {@link org.apache.curator.framework.state.CircuitBreakingConnectionStateListener}s
		 * via this mechanism by using
		 * {@link org.apache.curator.framework.state.ConnectionStateListenerDecorator#circuitBreaking(org.apache.curator.RetryPolicy)}
		 * or
		 * {@link org.apache.curator.framework.state.ConnectionStateListenerDecorator#circuitBreaking(org.apache.curator.RetryPolicy, java.util.concurrent.ScheduledExecutorService)}
		 *
		 * @param connectionStateListenerDecorator decorator to use
		 * @return this
		 * @since 4.2.0
		 */
		public Builder connectionStateListenerDecorator(
				ConnectionStateListenerDecorator connectionStateListenerDecorator) {
			this.connectionStateListenerDecorator = Objects.requireNonNull(connectionStateListenerDecorator,
