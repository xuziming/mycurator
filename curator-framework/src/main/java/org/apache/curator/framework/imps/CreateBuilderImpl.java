...
public class CreateBuilderImpl implements CreateBuilder, CreateBuilder2, BackgroundOperation<PathAndBytes>,
	...
	public CreateBuilderImpl(CuratorFrameworkImpl client, CreateMode createMode, Backgrounding backgrounding,
			boolean createParentsIfNeeded, boolean createParentsAsContainers, boolean doProtected, boolean compress,
	...
	public CreateBuilderMain withTtl(long ttl) {
		Preconditions.checkState(!client.isZk34CompatibilityMode(),
				"TTLs are not support when running in ZooKeeper 3.4 compatibility mode");
		this.ttl = ttl;
		return this;
	}

	<T> TransactionCreateBuilder<T> asTransactionCreateBuilder(final T context,
		...
		return new TransactionCreateBuilder<T>() {
			...
			public T forPath(String path, byte[] data) throws Exception {
				...
				if (client.isZk34CompatibilityMode()) {
					...
					transaction.add(Op.create(fixedPath, data, acling.getAclList(path), createMode, ttl),
							OperationType.CREATE, path);
				}
				return context;
			}
		...
		try {
			...
			final CompatibleCreateCallback mainCallback = new CompatibleCreateCallback() {
				...
				public void processResult(int rc, String path, Object ctx, String name, Stat stat) {
					...
					if ((rc == KeeperException.Code.NONODE.intValue()) && createParentsIfNeeded) {
						backgroundCreateParentsThenNode(client, operationAndData, operationAndData.getData().getPath(),
								backgrounding, acling.getACLProviderForParents(), createParentsAsContainers);
					} else if ((rc == KeeperException.Code.NODEEXISTS.intValue()) && setDataIfExists) {
						backgroundSetData(client, operationAndData, operationAndData.getData().getPath(),
								backgrounding);
					} else {
			...
			if (client.isZk34CompatibilityMode()) {
				...
				client.getZooKeeper().create(operationAndData.getData().getPath(), data,
						acling.getAclList(operationAndData.getData().getPath()), createMode, stringCallback,
						backgrounding.getContext());
			} else {
				CreateZK35.create(client.getZooKeeper(), operationAndData.getData().getPath(), data,
						acling.getAclList(operationAndData.getData().getPath()), createMode, mainCallback,
						backgrounding.getContext(), ttl);
			}
		} catch (Throwable e) {
	...
	static <T> void backgroundCreateParentsThenNode(final CuratorFrameworkImpl client,
			final OperationAndData<T> mainOperationAndData, final String path, Backgrounding backgrounding,
			final InternalACLProvider aclProvider, final boolean createParentsAsContainers) {
		...
		OperationAndData<T> parentOperation = new OperationAndData<>(operation, mainOperationAndData.getData(), null,
				null, backgrounding.getContext(), null);
		client.queueOperation(parentOperation);
	}

	private void backgroundSetData(final CuratorFrameworkImpl client,
		...
		BackgroundOperation<PathAndBytes> operation = new BackgroundOperation<PathAndBytes>() {
			...
			public void performBackgroundOperation(OperationAndData<PathAndBytes> op) throws Exception {
				try {
					client.getZooKeeper().setData(path, mainOperationAndData.getData().getData(),
							setDataIfExistsVersion, statCallback, backgrounding.getContext());
				} catch (KeeperException e) {
					// ignore
				}
			}
	...
	private void sendBackgroundResponse(int rc, String path, Object ctx, String name, Stat stat,
			OperationAndData<PathAndBytes> operationAndData) {
		path = client.unfixForNamespace(path);
		name = client.unfixForNamespace(name);

		CuratorEvent event = new CuratorEventImpl(client, CuratorEventType.CREATE, rc, path, name, ctx, stat, null,
				null, null, null, null);
		...
		OperationAndData<PathAndBytes> operationAndData = new OperationAndData<PathAndBytes>(this,
				...
				new OperationAndData.ErrorCallback<PathAndBytes>() {
					public void retriesExhausted(OperationAndData<PathAndBytes> operationAndData) {
						if (protectedMode.doProtected()) {
							// all retries have failed, findProtectedNodeInForeground(..) included, schedule
							// a clean up
			...
			void callPerformBackgroundOperation() throws Exception {
				...
				if (!localFirstTime && protectedMode.doProtected()) {
					...
					try {
						...
						sendBackgroundResponse(KeeperException.Code.CONNECTIONLOSS.intValue(), path,
								backgrounding.getContext(), null, null, this);
						callSuper = false;
					}
					if (createdPath != null) {
						try {
							sendBackgroundResponse(KeeperException.Code.OK.intValue(), createdPath,
									backgrounding.getContext(), createdPath, null, this);
						} catch (Exception e) {
							ThreadUtils.checkInterrupted(e);
				...
				if (failNextCreateForTesting) {
					...
					pathInForeground(path, data, acling.getAclList(path)); // simulate success on server without
				...
				if (createdPath == null) {
					try {
						...
						if (createParentsIfNeeded) {
							ZKPaths.mkdirs(client.getZooKeeper(), path, false, acling.getACLProviderForParents(),
									createParentsAsContainers);
							if (client.isZk34CompatibilityMode()) {
								createdPath = client.getZooKeeper().create(path, data, acling.getAclList(path),
										createMode);
							} else {
								createdPath = client.getZooKeeper().create(path, data, acling.getAclList(path),
										createMode, storingStat, ttl);
							}
						} else {
							throw e;
						}
	...
	private String findProtectedNodeInForeground(final String path) throws Exception {
		OperationTrace trace = client.getZookeeperClient()
				.startAdvancedTracer("CreateBuilderImpl-findProtectedNodeInForeground");
