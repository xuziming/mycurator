...
/**
 * manage the protected mode state for
 * {@link org.apache.curator.framework.imps.CreateBuilderImpl}
 */
class ProtectedMode {
	 ...
	 * @return either the found node or null - client should always use the returned
	 *         value
	 * @throws Exception errors
	 */
	String validateFoundNode(CuratorFrameworkImpl client, CreateMode createMode, String foundNode) throws Exception {
		if (doProtected && createMode.isEphemeral()) {
			...
			if (this.sessionId != clientSessionId) {
				log.info("Session has changed during protected mode with ephemeral. old: {} new: {}", this.sessionId,
						clientSessionId);
				if (foundNode != null) {
