...
public class TestEnsembleProvider extends BaseClassForTests {
	...
	public void testAfterSessionExpiration() throws Exception {
		...
		try {
			...
			for (int i = 0; i < 5; ++i) {
				// the ensemble provider should still be called periodically when the connection
				// is lost
	...
	private CuratorFramework newClient(Semaphore counter) {
		return CuratorFrameworkFactory.builder().ensembleProvider(new CountingEnsembleProvider(counter))
