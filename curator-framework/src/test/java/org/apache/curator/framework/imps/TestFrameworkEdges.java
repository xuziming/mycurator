...
public class TestFrameworkEdges extends BaseClassForTests {
	...
	public void testProtectionWithKilledSession() throws Exception {
		...
		// see CURATOR-498
		// attempt to re-create the state described in the bug report: create a 3
		// Instance ensemble;
		// have Curator connect to only 1 one of those instances; set
		// failNextCreateForTesting to
		// simulate protection mode searching; kill the connected server when this
		// happens;
		// wait for session timeout to elapse and then restart the instance. In most
		// cases
		// this will cause the scenario as Curator will send the session cancel and do
		// protection mode
		// search around the same time. The protection mode search should return first
		// as it can be resolved
		// by the Instance Curator is connected to but the session kill needs a quorum
		// vote (it's a
		// transaction)
		...
		try (TestingCluster cluster = new TestingCluster(3)) {
			...
			try (CuratorFramework client = CuratorFrameworkFactory.newClient(instanceSpec0.getConnectString(),
				...
				ErrorListenerPathAndBytesable<String> builder = client.create().withProtection()
						.withMode(CreateMode.EPHEMERAL).inBackground(callback);
	...
	public void testCreateContainersForBadConnect() throws Exception {
		...
		CuratorFramework client = CuratorFrameworkFactory.newClient(server.getConnectString(), 1000, 1000,
	...
	public void testQuickClose() throws Exception {
		CuratorFramework client = CuratorFrameworkFactory.newClient(server.getConnectString(), timing.session(), 1,
				new RetryNTimes(0, 0));
		try {
			client.start();
			client.close();
	...
	public void testProtectedCreateNodeDeletion() throws Exception {
		...
		try {
			...
			for (int i = 0; i < 2; ++i) {
				...
				Assert.assertEquals(children.size(), 0, children.toString()); // protected mode should have deleted the
																				// node
	...
	private void internalTestPathsFromProtectingInBackground(CreateMode mode) throws Exception {
		...
		try {
			...
			client = CuratorFrameworkFactory.newClient(server.getConnectString(), timing.session(), 1,
					new RetryOneTime(1));
			client.start();
	...
	public void testFailure() throws Exception {
		CuratorFramework client = CuratorFrameworkFactory.newClient(server.getConnectString(), 100, 100,
				new RetryOneTime(1));
		client.start();
		try {
			client.checkExists().forPath("/hey");
	...
	public void testRetry() throws Exception {
		...
		final CuratorFramework client = CuratorFrameworkFactory.newClient(server.getConnectString(), timing.session(),
				timing.connection(), new RetryOneTime(10));
		client.start();
		try {
	...
	public void testDeleteChildrenConcurrently() throws Exception {
		...
		try {
			...
			// The case run successfully, if client2 deleted a child successfully and the
			// client deleted children successfully
