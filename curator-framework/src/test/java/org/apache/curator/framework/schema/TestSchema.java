...
public class TestSchema extends BaseClassForTests {
	...
	public void testOrdering() throws Exception {
		...
		try {
			...
			client.create().creatingParentsIfNeeded().withMode(CreateMode.EPHEMERAL).forPath("/exact/foo/bar"); // schema
																												// "3"
		} finally {
			CloseableUtils.closeQuietly(client);
		}
	...
	private CuratorFramework newClient(SchemaSet schemaSet) {
		return CuratorFrameworkFactory.builder().connectString(server.getConnectString())
				.retryPolicy(new RetryOneTime(1)).schemaSet(schemaSet).build();
	}
