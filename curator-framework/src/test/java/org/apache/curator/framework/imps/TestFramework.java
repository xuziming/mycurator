...
public class TestFramework extends BaseClassForTests {
	...
	public void testWaitForShutdownTimeoutMs() throws Exception {
		...
		CuratorFramework client = CuratorFrameworkFactory.builder().connectString(server.getConnectString())
				.retryPolicy(new RetryOneTime(1)).zookeeperFactory(zookeeperFactory).waitForShutdownTimeoutMs(10064)
				.build();
		try {
			client.start();
	...
	public void testSessionLossWithLongTimeout() throws Exception {
		...
		try (final CuratorFramework client = CuratorFrameworkFactory.newClient(server.getConnectString(),
				timing.forWaiting().milliseconds(), timing.connection(), new RetryOneTime(1))) {
	...
	public void testOverrideCreateParentContainers() throws Exception {
		...
		CuratorFramework client = CuratorFrameworkFactory.builder().connectString(server.getConnectString())
				.retryPolicy(new RetryOneTime(1)).dontUseContainerParents().build();
		try {
			client.start();
