...
public class TestNamespaceFacade extends BaseClassForTests {
	...
	public void testInvalid() throws Exception {
		try {
			CuratorFrameworkFactory.builder().namespace("/snafu").retryPolicy(new RetryOneTime(1)).connectString("foo")
					.build();
			Assert.fail();
	...
	public void testGetNamespace() throws Exception {
		...
		CuratorFramework client2 = CuratorFrameworkFactory.builder().namespace("snafu").retryPolicy(new RetryOneTime(1))
	...
	public void testIsStarted() throws Exception {
		...
		Assert.assertEquals(client.getState(), namespaced.getState(),
	...
	public void testACL() throws Exception {
		...
		client.setACL().withACL(Collections.singletonList(new ACL(ZooDefs.Perms.WRITE, ZooDefs.Ids.ANYONE_ID_UNSAFE)))
				.forPath("/parent/child");
		// This will attempt to setACL on /parent/child, Previously this failed because
		// /child
		// isn't present. Using "child" would case a failure because the path didn't
		// start with
		// a slash
		try {
	...
	public void testUnfixForEmptyNamespace() {
		CuratorFramework client = CuratorFrameworkFactory.builder().namespace("").retryPolicy(new RetryOneTime(1))
				.connectString("foo").build();
