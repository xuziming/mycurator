...
public class TestCompressionInTransactionOld extends BaseClassForTests {
	...
	public void testSetCompressedAndUncompressed() throws Exception {
		...
		try {
			...
			client.inTransaction().setData().compressed().forPath(path1, data1).and().setData().forPath(path2, data2)
					.and().commit();
	...
	public void testSimple() throws Exception {
		...
		try {
			...
			client.inTransaction().create().compressed().forPath(path1, data1).and().create().compressed()
					.forPath(path2, data2).and().commit();
	...
	/**
	 * Test the case where both uncompressed and compressed data is generated in the
	 * same transaction
	 * 
	 * @throws Exception
	 */
	...
	public void testCreateCompressedAndUncompressed() throws Exception {
		...
		try {
			...
			client.inTransaction().create().compressed().forPath(path1, data1).and().create().forPath(path2, data2)
					.and().commit();
