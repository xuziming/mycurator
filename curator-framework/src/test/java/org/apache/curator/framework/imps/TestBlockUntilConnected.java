...
public class TestBlockUntilConnected extends BaseClassForTests {
	...
	public void testBlockUntilConnectedCurrentlyAwaitingReconnect() {
		Timing timing = new Timing();
		CuratorFramework client = CuratorFrameworkFactory.builder().connectString(server.getConnectString())
				.sessionTimeoutMs(timing.session()).retryPolicy(new RetryOneTime(1)).build();
	...
	/**
	 * Test the case where we are not currently connected and the thread gets
	 * interrupted prior to a connection becoming available
	 */
	@Test
	...
	/**
	 * Test that we are actually connected every time that we block until connection
	 * is established in a tight loop.
	 */
	@Test
