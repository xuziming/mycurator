...
public class TestRetryLoop extends BaseClassForTests {
	...
	public void testRetryLoopWithFailure() throws Exception {
		CuratorZookeeperClient client = new CuratorZookeeperClient(server.getConnectString(), 5000, 5000, null,
				new RetryOneTime(1));
		client.start();
		try {
			int loopCount = 0;
			...
			outer: while (retryLoop.shouldContinue()) {
				...
				try {
					...
					client.getZooKeeper().create("/test", new byte[] { 1, 2, 3 }, ZooDefs.Ids.OPEN_ACL_UNSAFE,
