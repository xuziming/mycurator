...
public class ZKPaths {
	...
	/**
	 * @return {@link CreateMode#CONTAINER} if the ZK JAR supports it. Otherwise
	...
	/**
	 * Given a full path, return the node name and its path. i.e. "/one/two/three"
	 ...
	 * currently perform validation on the provided path; it will just return a
	 * string comprising the last ten characters.
	 * 
	...
	/**
	 * Given a full path, return the the individual parts, without slashes. The root
	 * path will return an empty list.
	 *
	 * @param path the path
	...
	/**
	 * Make sure all the nodes in the path are created. NOTE: Unlike File.mkdirs(),
	 * Zookeeper doesn't distinguish between directories and files. So, every node
	 * in the path is created. The data for each node is an empty blob
	 *
	 ...
	 * @param zookeeper    the client
	 * @param path         path to ensure
	 * @param makeLastNode if true, all nodes are created. If false, only the parent
	 *                     nodes are created
	 * @throws InterruptedException                 thread interruption
	 * @throws org.apache.zookeeper.KeeperException Zookeeper errors
	 */
	 ...
	 * @param makeLastNode if true, all nodes are created. If false, only the parent
	 *                     nodes are created
	 * @param aclProvider  if not null, the ACL provider to use when creating parent
	 *                     nodes
	 * @throws InterruptedException                 thread interruption
	 * @throws org.apache.zookeeper.KeeperException Zookeeper errors
	 */
	 ...
	 * @param aclProvider  if not null, the ACL provider to use when creating parent
	 *                     nodes
	 * @param asContainers if true, nodes are created as
	...
	public static String makePath(String parent, String child) {
		...
		// Avoid internal StringBuilder's buffer reallocation by specifying the max path
		// length
	...
	/**
	 * Given a parent and a child node, join them in the given {@link StringBuilder
	 * path}
	 *
