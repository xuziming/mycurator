...
public class CuratorZookeeperClient implements Closeable {
	 ...
	 */
	public CuratorZookeeperClient(String connectString, int sessionTimeoutMs, int connectionTimeoutMs, Watcher watcher,
			RetryPolicy retryPolicy) {
		this(new DefaultZookeeperFactory(), new FixedEnsembleProvider(connectString), sessionTimeoutMs,
				connectionTimeoutMs, watcher, retryPolicy, false, new StandardConnectionHandlingPolicy());
	}
	...
	public CuratorZookeeperClient(EnsembleProvider ensembleProvider, int sessionTimeoutMs, int connectionTimeoutMs,
		...
		this(new DefaultZookeeperFactory(), ensembleProvider, sessionTimeoutMs, connectionTimeoutMs, watcher,
				retryPolicy, false, new StandardConnectionHandlingPolicy());
	}
	 ...
	 * @param watcher             default watcher or null
	 * @param retryPolicy         the retry policy to use
	 * @param canBeReadOnly       if true, allow ZooKeeper client to enter read only
	 *                            mode in case of a network partition. See
	 *                            {@link ZooKeeper#ZooKeeper(String, int, Watcher, long, byte[], boolean)}
	 *                            for details
	 */
	public CuratorZookeeperClient(ZookeeperFactory zookeeperFactory, EnsembleProvider ensembleProvider,
			int sessionTimeoutMs, int connectionTimeoutMs, Watcher watcher, RetryPolicy retryPolicy,
			boolean canBeReadOnly) {
		this(zookeeperFactory, ensembleProvider, sessionTimeoutMs, connectionTimeoutMs, watcher, retryPolicy,
				canBeReadOnly, new StandardConnectionHandlingPolicy());
	}

	/**
	 * @param zookeeperFactory         factory for creating {@link ZooKeeper}
	 *                                 instances
	 * @param ensembleProvider         the ensemble provider
	 * @param sessionTimeoutMs         session timeout
	 * @param connectionTimeoutMs      connection timeout
	 * @param watcher                  default watcher or null
	 * @param retryPolicy              the retry policy to use
	 * @param canBeReadOnly            if true, allow ZooKeeper client to enter read
	 *                                 only mode in case of a network partition. See
	 *                                 {@link ZooKeeper#ZooKeeper(String, int, Watcher, long, byte[], boolean)}
	 *                                 for details
	 * @param connectionHandlingPolicy connection handling policy - use one of the
	 *                                 pre-defined policies or write your own
	 * @since 3.0.0
	 */
	public CuratorZookeeperClient(ZookeeperFactory zookeeperFactory, EnsembleProvider ensembleProvider,
			int sessionTimeoutMs, int connectionTimeoutMs, Watcher watcher, RetryPolicy retryPolicy,
			boolean canBeReadOnly, ConnectionHandlingPolicy connectionHandlingPolicy) {
		this(zookeeperFactory, ensembleProvider, sessionTimeoutMs, connectionTimeoutMs, 0, watcher, retryPolicy,
				canBeReadOnly, connectionHandlingPolicy);
	}
	...
	public CuratorZookeeperClient(ZookeeperFactory zookeeperFactory, EnsembleProvider ensembleProvider,
		...
		if (sessionTimeoutMs < connectionTimeoutMs) {
			log.warn(String.format("session timeout [%d] is less than connection timeout [%d]", sessionTimeoutMs,
					connectionTimeoutMs));
		}
		...
		state = new ConnectionState(zookeeperFactory, ensembleProvider, sessionTimeoutMs, connectionTimeoutMs, watcher,
				tracer, canBeReadOnly, connectionHandlingPolicy);
	 ...
	 * @throws Exception if the connection timeout has elapsed or an exception
	 *                   occurs in a background process
	 */
	...
	/**
	 * Return a new "session fail" retry loop. See {@link SessionFailRetryLoop} for
	...
	/**
	 * This method blocks until the connection to ZK succeeds. Use with caution. The
	 * block will timeout after the connection timeout (as passed to the
	...
	/**
	 * Close this client object as the {@link #close() } method. This method will
	 * wait for internal resources to be released.
	 * 
	 * @param waitForShutdownTimeoutMs timeout (in milliseconds) to wait for
	 *                                 resources to be released. Use zero or a
	 *                                 negative value to skip the wait.
	 */
