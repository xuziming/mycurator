...
class HandleHolder {
	...
	HandleHolder(ZookeeperFactory zookeeperFactory, Watcher watcher, EnsembleProvider ensembleProvider,
	...
	String getNewConnectionString() {
		...
		return ((helperConnectionString != null)
				&& !ensembleProvider.getConnectionString().equals(helperConnectionString)) ? helperConnectionString
						: null;
	}
	...
	private void internalClose(int waitForShutdownTimeoutMs) throws Exception {
		try {
			...
			if (zooKeeper != null) {
				...
				zooKeeper.register(dummyWatcher); // clear the default watcher so that no new events get processed by
													// mistake
				if (waitForShutdownTimeoutMs == 0) {
					zooKeeper.close(); // coming from closeAndReset() which is executed in ZK's event thread. Cannot
										// use zooKeeper.close(n) otherwise we'd get a dead lock
				} else {
