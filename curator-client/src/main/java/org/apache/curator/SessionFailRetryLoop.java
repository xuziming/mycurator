...
/**
 * <p>
 * See {@link RetryLoop} for the main details on retry loops. <b>All
 * Curator/ZooKeeper operations should be done in a retry loop.</b>
 * </p>
 *
 * <p>
 * The standard retry loop treats session failure as a type of connection
 * failure. i.e. the fact that it is a session failure isn't considered. This
 * can be problematic if you are performing a series of operations that rely on
 * ephemeral nodes. If the session fails after the ephemeral node has been
 * created, future Curator/ZooKeeper operations may succeed even though the
 ...
 * <li>In most cases, the reconnect will succeed and, thus, the node creation
 * will succeed even though the ephemeral node will have been deleted by
 * ZooKeeper.</li>
 * </ul>
 * </li>
 * </ul>
 *
 * <p>
 * The SessionFailRetryLoop prevents this type of scenario. When a session
 * failure is detected, the thread is marked as failed which will cause all
 * future Curator operations to fail. The SessionFailRetryLoop will then either
 * retry the entire set of operations or fail (depending on
 * {@link SessionFailRetryLoop.Mode})
 * </p>
 *
 * Canonical usage:<br>
 * 
...
public class SessionFailRetryLoop implements Closeable {
	...
	private static final Set<Thread> failedSessionThreads = Sets
			.newSetFromMap(Maps.<Thread, Boolean>newConcurrentMap());
	...
	public enum Mode {
		...
		/**
		 * If the session fails, throw {@link KeeperException.SessionExpiredException}
		 * when {@link SessionFailRetryLoop#shouldContinue()} is called
		 */
		FAIL
	}

	/**
	 * Convenience utility: creates a "session fail" retry loop calling the given
	 * proc
	 *
