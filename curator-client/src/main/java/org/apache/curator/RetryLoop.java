 ...
 * check with the current retry policy and either attempt to reconnect or
 ...
 * 	try {
 * 		// do your work
 * 		ZooKeeper zk = client.getZooKeeper(); // it's important to re-get the ZK instance in case there was an error
...
public class RetryLoop {
	...
	/**
	 * Convenience utility: creates a retry loop calling the given proc and retrying
	 * if needed
	 *
	...
	public static boolean shouldRetry(int rc) {
		return (rc == KeeperException.Code.CONNECTIONLOSS.intValue())
				...
				|| (rc == KeeperException.Code.SESSIONEXPIRED.intValue()) || (rc == -13); // KeeperException.Code.NEWCONFIGNOQUORUM.intValue())
