...
/**
 * Ensemble provider that polls a cluster of Exhibitor
 * (https://github.com/Netflix/exhibitor) instances for the connection string.
 * If the set of instances should change, new ZooKeeper connections will use the
...
public class ExhibitorEnsembleProvider implements EnsembleProvider {
	...
	private final ScheduledExecutorService service = ThreadUtils
			.newSingleThreadScheduledExecutor("ExhibitorEnsembleProvider");
	...
	/**
	 * @param exhibitors  the current set of exhibitor instances (can be changed
	 *                    later via {@link #setExhibitors(Exhibitors)})
	 * @param restClient  the rest client to use (use
	 *                    {@link DefaultExhibitorRestClient} for most cases)
	 * @param restUriPath the path of the REST call used to get the server set.
	 *                    Usually: <code>/exhibitor/v1/cluster/list</code>
	...
	/**
	 * Can be called prior to starting the Curator instance to set the current
	 * connection string
	 *
	...
	protected void poll() {
		...
		if (count > 0) {
			...
			if (!newConnectionStringValue.equals(connectionString.get())) {
				log.info(String.format("Connection string has changed. Old value (%s), new value (%s)",
			...
			Exhibitors newExhibitors = new Exhibitors(newHostnames, localExhibitors.getRestPort(),
					new Exhibitors.BackupConnectionStringProvider() {
						...
						public String getBackupConnectionString() throws Exception {
							return masterExhibitors.get().getBackupConnectionString(); // this may be overloaded by
																						// clients. Make sure there is
																						// always a method call to get
																						// the value.
						}
					});
