...
public class ServiceInstance<T> {
	...
	/**
	 * Return a new builder. The {@link #address} is set to the ip of the first NIC
	 * in the system. The {@link #id} is set to a random UUID.
	 *
	 * @return builder
	...
	public ServiceInstance(String name, String id, String address, Integer port, Integer sslPort, T payload,
			long registrationTimeUTC, ServiceType serviceType, UriSpec uriSpec) {
		this(name, id, address, port, sslPort, payload, registrationTimeUTC, serviceType, uriSpec, true);
	}

	/**
	 * IMPORTANT: Due to CURATOR-275 the <code>enabled</code> field is
	 * <strong>NOT</strong> supported by default. If you wish to use the enabled
	 * field, you must set a
	 * {@link org.apache.curator.x.discovery.details.InstanceSerializer} that
	 * serializes this field. The default serializer,
	 * {@link org.apache.curator.x.discovery.details.JsonInstanceSerializer} does
	 * not serialize the field by default. You must use the alternate constructor
	 * {@link org.apache.curator.x.discovery.details.JsonInstanceSerializer#JsonInstanceSerializer(Class, boolean)}
	 ...
	 */
	public ServiceInstance(String name, String id, String address, Integer port, Integer sslPort, T payload,
			long registrationTimeUTC, ServiceType serviceType, UriSpec uriSpec, boolean enabled) {
	...
	public String toString() {
		return "ServiceInstance{" + "name='" + name + '\'' + ", id='" + id + '\'' + ", address='" + address + '\''
				...
				+ registrationTimeUTC + ", serviceType=" + serviceType + ", uriSpec=" + uriSpec + ", enabled=" + enabled
				+ '}';
	}
}
