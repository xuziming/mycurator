...
public class TestServiceCache extends BaseClassForTests {
	...
	public void testInitialLoad() throws Exception {
		...
		try {
			...
			ServiceInstance<String> instance2 = ServiceInstance.<String>builder().payload("test").name("test")
					.port(10065).build();
			ServiceInstance<String> instance3 = ServiceInstance.<String>builder().payload("test").name("test")
					.port(10066).build();
	...
	public void testViaProvider() throws Exception {
		...
		try {
			...
			ServiceInstance<String> instance2 = ServiceInstance.<String>builder().address("foo").payload("thing")
					.name("test").port(10064).build();
			discovery.registerService(instance2);
			timing.sleepABit();
	...
	public void testUpdate() throws Exception {
		...
		try {
			...
			instance = ServiceInstance.<String>builder().payload("changed").name("test").port(10064)
					.id(instance.getId()).build();
	...
	public void testCache() throws Exception {
		...
		try {
			...
			ServiceInstance<String> instance3 = ServiceInstance.<String>builder().payload("thing").name("another")
					.port(10064).build();
			discovery.registerService(instance3);
			Assert.assertFalse(semaphore.tryAcquire(3, TimeUnit.SECONDS)); // should not get called for a different
																			// service
		} finally {
			Collections.reverse(closeables);
			for (Closeable c : closeables) {
	...
	public void testExecutorServiceIsInvoked() throws Exception {
		...
		try {
			...
			ExecuteCalledWatchingExecutorService exec = new ExecuteCalledWatchingExecutorService(
