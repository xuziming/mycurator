...
class ModeledCacheImpl<T> implements TreeCacheListener, ModeledCache<T> {
	...
	ModeledCacheImpl(CuratorFramework client, ModelSpec<T> modelSpec, ExecutorService executor) {
		...
		cache = TreeCache.newBuilder(client, basePath.fullPath()).setCacheData(false)
				.setDataIsCompressed(modelSpec.createOptions().contains(CreateOption.compress)).setExecutor(executor)
	...
	public Map<ZPath, ZNode<T>> currentChildren(ZPath path) {
		return entries.entrySet().stream().filter(entry -> entry.getKey().startsWith(path))
				.map(entry -> new AbstractMap.SimpleEntry<>(entry.getKey(),
						new ZNodeImpl<>(entry.getKey(), entry.getValue().stat, entry.getValue().model)))
