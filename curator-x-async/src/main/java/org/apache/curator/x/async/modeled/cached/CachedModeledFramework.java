...
public interface CachedModeledFramework<T> extends ModeledFramework<T>, Closeable {
	...
	/**
	 * Same as
	 * {@link org.apache.curator.x.async.modeled.ModeledFramework#childrenAsZNodes()}
	 * but always reads from cache - i.e. no additional queries to ZooKeeper are
	 * made
	 *
	...
	/**
	 * Same as {@link #read()} except that if the cache does not have a value for
	 * this path a direct query is made.
	 *
	 * @return AsyncStage
	...
	/**
	 * Same as {@link #read(org.apache.zookeeper.data.Stat)} except that if the
	 * cache does not have a value for this path a direct query is made.
	 *
