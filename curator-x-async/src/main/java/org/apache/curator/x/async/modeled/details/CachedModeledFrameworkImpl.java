...
class CachedModeledFrameworkImpl<T> implements CachedModeledFramework<T> {
	...
	public AsyncStage<List<T>> list() {
		List<T> children = cache.currentChildren().values().stream().map(ZNode::model).collect(Collectors.toList());
	...
	private <U> AsyncStage<U> exceptionally() {
		KeeperException.NoNodeException exception = new KeeperException.NoNodeException(
