...
public class ModelSpecImpl<T> implements ModelSpec<T>, SchemaValidator {
	...
	public ModelSpecImpl(ZPath path, ModelSerializer<T> serializer, CreateMode createMode, List<ACL> aclList,
			Set<CreateOption> createOptions, Set<DeleteOption> deleteOptions, long ttl) {
	...
	public Schema schema() {
		if (schema == null) {
			schema = Schema.builder(path.toSchemaPathPattern()).dataValidator(this)
					...
					.watched(Schema.Allowance.CAN).build();
		}
		return schema;
	}
	...
	public boolean isValid(Schema schema, String path, byte[] data, List<ACL> acl) {
		if (acl != null) {
			...
			if (!acl.equals(localAclList)) {
				throw new SchemaViolation(schema, new SchemaViolation.ViolatorData(path, data, acl),
						"ACLs do not match model ACLs");
			}
		}
