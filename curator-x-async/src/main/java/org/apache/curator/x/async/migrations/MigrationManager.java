...
public class MigrationManager {
	 ...
	 */
	public MigrationManager(AsyncCuratorFramework client, String lockPath, String metaDataPath, Executor executor,
			Duration lockMax) {
	 ...
	 *
	 * @param set the set
	 * @return completion stage. If there is a migration-specific error, the stage
	 *         will be completed exceptionally with
	 *         {@link org.apache.curator.x.async.migrations.MigrationException}.
	 */
	...
	/**
	 * Can be overridden to change how the comparison to previous migrations is
	 * done. The default version ensures that the meta data from previous migrations
	 * matches the current migration set exactly (by order and version). If there is
	 * a mismatch, <code>MigrationException</code> is thrown.
	 *
	 * @param set                    the migration set being applied
	 * @param operationHashesInOrder previous operation hashes (may be empty)
	 * @return the list of actual migrations to perform. The filter can return any
	 *         value here or an empty list.
	...
	protected List<Migration> filter(MigrationSet set, List<byte[]> operationHashesInOrder) throws MigrationException {
		if (operationHashesInOrder.size() > set.migrations().size()) {
			throw new MigrationException(set.id(),
					String.format("More metadata than migrations. Migration ID: %s", set.id()));
		}
	...
	private CompletionStage<Void> runMigrationInLock(InterProcessLock lock, MigrationSet set) {
		...
		return childrenWithData(client, thisMetaDataPath)
				.thenCompose(metaData -> applyMetaData(set, metaData, thisMetaDataPath)).handle((v, e) -> {
					release(lock, true);
					if (e != null) {
	...
	private CompletionStage<Void> applyMetaData(MigrationSet set, Map<String, byte[]> metaData,
			String thisMetaDataPath) {
		List<byte[]> sortedMetaData = metaData.keySet().stream().sorted(Comparator.naturalOrder()).map(metaData::get)
	...
	private CompletionStage<Void> applyMetaDataAfterEnsure(List<Migration> toBeApplied, String thisMetaDataPath) {
		...
		toBeApplied.forEach(migration -> {
			...
			operations.add(client.transactionOp().create().withMode(CreateMode.PERSISTENT_SEQUENTIAL)
