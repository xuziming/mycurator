...
public interface ModeledFramework<T> {
	...
	/**
	 * Start a new ModeledFrameworkBuilder. A client and model must be provided
	 * prior to the instance being built via
	 * {@link org.apache.curator.x.async.modeled.ModeledFrameworkBuilder#withClient(org.apache.curator.x.async.AsyncCuratorFramework)}
	 * and
	 * {@link org.apache.curator.x.async.modeled.ModeledFrameworkBuilder#withModelSpec(ModelSpec)}
	 *
	 * @return builder
	 */
	...
	/**
	 * <p>
	 * Use an internally created cache as a front for this modeled instance. All
	 * read APIs use the internal cache. i.e. read calls always use the cache
	 * instead of making direct queries. Note: you must call
	 * {@link org.apache.curator.x.async.modeled.cached.CachedModeledFramework#start()}
	 * and
	 * {@link org.apache.curator.x.async.modeled.cached.CachedModeledFramework#close()}
	 * to start/stop
	 * </p>
	 *
	 * <p>
	 * Note: the root node (the main path of the model) is <em>not</em> cached. i.e.
	 * only nodes below the root are cached.
	 * </p>
	 *
	 * <p>
	 * Note: this method internally allocates an Executor for the cache and read
	 * methods. Use {@link #cached(java.util.concurrent.ExecutorService)} if you'd
	 * like to provide your own executor service.
	 * </p>
	 *
	 ...
	 * {@link org.apache.curator.x.async.modeled.versioned.Versioned} containers
	 *
	 * @return wrapped instance
	 */
	 ...
	 * {@link #wrap(org.apache.curator.x.async.AsyncCuratorFramework, ModelSpec)} or
	 * the builder.
	 *
	...
	/**
	 * <p>
	 * Return a new Modeled Curator instance with all the same options but applying
	 * to the given child node of this Modeled Curator's path. E.g. if this Modeled
	 * Curator instance applies to "/a/b", calling <code>modeled.at("c")</code>
	 * returns an instance that applies to "/a/b/c".
	 * </p>
	 *
	 * <p>
	 * The replacement is the <code>toString()</code> value of child or, if it
	 * implements {@link org.apache.curator.x.async.modeled.NodeName}, the value of
	 * <code>nodeName()</code>.
	 * </p>
	 *
	 ...
	 * <p>
	 * Return a new Modeled Curator instance with all the same options but applying
	 * to the parent node of this Modeled Curator's path. E.g. if this Modeled
	 * Curator instance applies to "/a/b/c", calling <code>modeled.parent()</code>
	 * returns an instance that applies to "/a/b".
	 * </p>
	 *
	 * <p>
	 * The replacement is the <code>toString()</code> value of child or, if it
	...
	/**
	 * Return a Modeled Curator instance with all the same options but using the
	 * given path.
	 *
	 * @param path new path
	 * @return new Modeled Curator instance
	...
	/**
	 * Create (or update depending on build options) a ZNode at this instance's path
	 * with a serialized version of the given model
	 *
	 ...
	 *
	 * @param model   model to write
	 * @param version if data is being set instead of creating the node, the data
	 *                version to use
	 * @return AsyncStage
	 * @see org.apache.curator.x.async.AsyncStage
	 */
	...
	/**
	 * Create (or update depending on build options) a ZNode at this instance's path
	 * with a serialized form of the given model
	 *
	 * @param model         model to write
	...
	/**
	 * Update the ZNode at this instance's path with a serialized form of the given
	 * model passing "-1" for the update version
	 *
	 * @param model model to write
	 * @return AsyncStage
	 * @see org.apache.curator.x.async.AsyncStage
	...
	/**
	 * Update the ZNode at this instance's path with a serialized form of the given
	 * model passing the given update version
	 *
	 * @param model   model to write
	...
	/**
	 * Return the child paths of this instance's path (in no particular order) and
	 * deserialize into a models. IMPORTANT: this results in a ZooKeeper query for
	 * each child node returned. i.e. if the initial children() call returns 10
	 * nodes an additional 10 ZooKeeper queries are made to get the data. Note:
	 ...
	 * transaction. Note: due to ZooKeeper transaction limits, this is a _not_ a
	 * "set or update" operation but only a create operation and will generate an
