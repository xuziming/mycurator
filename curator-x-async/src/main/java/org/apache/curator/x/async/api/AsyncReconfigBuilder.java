...
public interface AsyncReconfigBuilder {
	/**
	 * Sets one or more members that are meant to be the ensemble. The expected
	 * format is:
	 * <code>server.[id]=[hostname]:[peer port]:[election port]:[type];[client port]</code>
	 *
	...
	/**
	 * Adds servers to join the ensemble and/or servers to leave the ensemble. The
	 * format for <strong>joining</strong> is:
	 * <code>server.[id]=[hostname]:[peer port]:[election port]:[type];[client port]</code>.
	 * The format for <strong>leaving</strong> is a list of server IDs.
	 *
	...
	/**
	 * Same as {@link #withNewMembers(java.util.List)} but allows specified the
	 * configuration version to use. By default the configuration version is -1.
	 *
	 * @param servers    The new server list
	...
	/**
	 * Specify joiners, leaves and config version. By default the configuration
	 * version is -1.
	 *
	 * @param joining    The servers joining
	 * @param leaving    The servers leaving
	...
	/**
	 * Same as {@link #withNewMembers(java.util.List)} but allows a stat to hold the
	 * stat info from "/zookeeper/config"
	 *
	...
	/**
	 * Same as {@link #withJoiningAndLeaving(java.util.List, java.util.List)} but
	 * allows a stat to hold the stat info from "/zookeeper/config"
	 *
	 * @param joining The servers joining
	 * @param leaving The servers leaving
	 * @param stat    stat to hold the stat value
	...
	/**
	 * Same as {@link #withJoiningAndLeaving(java.util.List, java.util.List)} with
	 * stat and config version
	 *
	 * @param joining    The servers joining
	 * @param leaving    The servers leaving
	 * @param stat       stat to hold the stat value
