...
public class AsyncWrappers {
	 ...
	 * for each node. IMPORTANT: this results in a ZooKeeper query for each child
	 * node returned. i.e. if the initial children() call returns 10 nodes an
	 * additional 10 ZooKeeper queries are made to get the data.
	 * </p>
	 *
	 * <p>
	 * Note: if the any of the nodes in the path do not exist yet,
	 * {@link org.apache.zookeeper.KeeperException.NoNodeException} is
	 * <strong>NOT</strong> set. Instead the stage is completed with an empty map.
	 * </p>
	 *
	...
	/**
	 * Asynchronously ensure that the parents of the given path are created as
	 * containers
	 *
	 * @param client client
	 * @param path   path to ensure
	...
	/**
	 * Set as the completion stage's exception when trying to acquire a lock times
	 * out
	 */
	public static class TimeoutException extends RuntimeException {
		private static final long serialVersionUID = -1593707389877407208L;
	}

	/**
	 * Attempt to acquire the given lock asynchronously using the given timeout and
	 * executor. If the lock is not acquired within the timeout stage is
	 * completedExceptionally with {@link AsyncWrappers.TimeoutException}
	 *
	 * @param lock     a lock implementation (e.g.
	 *                 {@link org.apache.curator.framework.recipes.locks.InterProcessMutex},
	 *                 {@link org.apache.curator.framework.recipes.locks.InterProcessSemaphoreV2},
	 *                 etc.)
	...
	/**
	 * Attempt to acquire the given lock asynchronously using the given timeout and
	 * executor. The stage is completed with a Boolean that indicates whether or not
	...
	/**
	 * Attempt to acquire the given lock asynchronously using the given executor and
	 * without a timeout.
	 *
	 * @param lock     a lock implementation (e.g.
	 *                 {@link org.apache.curator.framework.recipes.locks.InterProcessMutex},
	...
	/**
	 * Attempt to acquire the given lock asynchronously using the given timeout
	 * using the {@link java.util.concurrent.ForkJoinPool#commonPool()}. If the lock
	 * is not acquired within the timeout stage is completedExceptionally with
	 * {@link AsyncWrappers.TimeoutException}
	 *
	 * @param lock    a lock implementation (e.g.
	 *                {@link org.apache.curator.framework.recipes.locks.InterProcessMutex},
	 *                {@link org.apache.curator.framework.recipes.locks.InterProcessSemaphoreV2},
	...
	/**
	 * Attempt to acquire the given lock asynchronously using the given timeout
	 * using the {@link java.util.concurrent.ForkJoinPool#commonPool()}. The stage
	 * is completed with a Boolean that indicates whether or not the lock was
	 * acquired.
	 *
	 * @param lock    a lock implementation (e.g.
	 *                {@link org.apache.curator.framework.recipes.locks.InterProcessMutex},
	 ...
	 *
	 * @param lock                   lock to release
	 * @param ignoreNoLockExceptions if true {@link java.lang.IllegalStateException}
	 *                               is ignored
	 */
	...
	private static void completeChildren(AsyncCuratorFramework client, CompletableFuture<Map<String, byte[]>> future,
			String parentPath, List<String> children, boolean isCompressed) {
		...
		children.forEach(node -> {
			...
			AsyncStage<byte[]> stage = isCompressed ? client.getData().decompressed().forPath(path)
