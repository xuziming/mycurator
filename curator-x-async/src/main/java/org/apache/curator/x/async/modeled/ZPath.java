...
public interface ZPath extends Resolvable {
	 ...
	 * via {@link #resolved(Object...)} when passed via the various
	 * <code>from()</code> methods
	 */
	...
	/**
	 * Same as {@link #parameter()} but allows you to specify an alternate
	 * code/name. This name has no effect and is only for debugging purposes. When
	 * <code>toString()</code> is called on ZPaths, this code shows.
	 */
	...
	/**
	 * Take a string path and return a ZPath. Each part of the path that is
	 * <code>{XXXX}</code> is replaced with {@link #parameter()}. E.g.
	 * <code>parseWithIds("/one/two/{first}/three/{second}")</code> is the
	 * equivalent of calling
	 * <code>ZPath.from("one", "two", parameter(), "three", parameter())</code>
	 *
	 * @param fullPath the path to parse
	 * @return ZPath
	 * @throws IllegalArgumentException if the path is invalid
	 ...
	 * <code>ZPath.from("my", "full", "path")</code>. Any/all of the names can be
	 * passed as {@link #parameter()} so that the path can be resolved later using
	...
	/**
	 * Convert individual path names into a ZPath. Any/all of the names can be
	 * passed as {@link #parameter()} so that the path can be resolved later using
	 * of the <code>resolved()</code> methods.
	 *
	 * @param names path names
	 ...
	 * {@link #parameter()} so that the path can be resolved later using of the
	 * <code>resolved()</code> methods.
	 *
	 ...
	 * Any/all of the names can be passed as {@link #parameter()} so that the path
	 * can be resolved later using of the <code>resolved()</code> methods.
	 *
	 * @param base  base/starting path
	 * @param names path names
	 * @return ZPath
	 ...
	 * {@link org.apache.curator.framework.schema.Schema}
	 *
