...
public interface AsyncCuratorFramework extends AsyncCuratorFrameworkDsl {
	 ...
	 * leads back to {@link org.apache.curator.framework.CuratorFrameworkFactory}.
	 * i.e. you can have derived instances such as
	 * {@link org.apache.curator.framework.WatcherRemoveCuratorFramework} etc. but
	 * the original client must have been created by the Factory.
	 *
	 ...
	 * {@link #wrap(org.apache.curator.framework.CuratorFramework)}
	 *
	...
	/**
	 * Returns a facade that changes how watchers are set when {@link #watched()} is
	 * called
	 *
	...
	/**
	 * Returns a facade that adds the given UnhandledErrorListener to all background
	 * operations
	 *
	...
	/**
	 * Returns a facade that adds the the given filters to all background operations
	 * and watchers. <code>resultFilter</code> will get called for every background
	 * callback. <code>watcherFilter</code> will get called for every watcher. The
	 * filters can return new versions or unchanged versions of the arguments.
	 *
	 ...
	 * @return facade
	 */
	AsyncCuratorFrameworkDsl with(UnhandledErrorListener listener, UnaryOperator<CuratorEvent> resultFilter,
			UnaryOperator<WatchedEvent> watcherFilter);

	/**
	 * Set any combination of listener, filters or watch mode
	 *
	 * @param mode          watch mode to use for subsequent calls to
	 *                      {@link #watched()} (cannot be <code>null</code>)
	 * @param listener      lister to use or <code>null</code>
	 * @param resultFilter  filter to use or <code>null</code>
	 * @param watcherFilter filter to use or <code>null</code>
	 * @see #with(WatchMode)
	 ...
	 * @return facade
	 */
	AsyncCuratorFrameworkDsl with(WatchMode mode, UnhandledErrorListener listener,
