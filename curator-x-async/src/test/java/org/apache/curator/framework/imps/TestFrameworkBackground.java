...
public class TestFrameworkBackground extends BaseClassForTests {
	...
	public void testErrorListener() throws Exception {
		...
		CuratorFramework client = CuratorFrameworkFactory.builder().connectString(server.getConnectString())
				.retryPolicy(new RetryOneTime(1)).aclProvider(badAclProvider).build();
		try {
			client.start();
	...
	public void testListenerConnectedAtStart() throws Exception {
		...
		try {
			...
			// due to CURATOR-72, this was causing a LOST event to precede the CONNECTED
			// event
			...
			Assert.assertEquals(firstconnectionState, ConnectionState.CONNECTED,
	...
	/**
	 * Attempt a background operation while Zookeeper server is down. Return code
	 * must be {@link org.apache.zookeeper.KeeperException.Code#CONNECTIONLOSS}
	 */
	@Test
	...
	/**
	 * CURATOR-126 Shutdown the Curator client while there are still background
	 * operations running.
	 */
	@Test
	public void testShutdown() throws Exception {
		...
		CuratorFramework client = CuratorFrameworkFactory.builder().connectString(server.getConnectString())
				.sessionTimeoutMs(timing.session()).connectionTimeoutMs(timing.connection())
				.retryPolicy(new RetryOneTime(1)).maxCloseWaitMs(timing.forWaiting().milliseconds()).build();
		try {
