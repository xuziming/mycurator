...
public class TestModeledFramework extends TestModeledFrameworkBase {
	...
	public void testBackwardCompatibility() {
		TestNewerModel rawNewModel = new TestNewerModel("John", "Galt", "1 Galt's Gulch", 42, BigInteger.valueOf(1),
				100);
	...
	public void testSchema() throws Exception {
		...
		try (CuratorFramework schemaClient = CuratorFrameworkFactory.builder().connectString(server.getConnectString())
				.retryPolicy(new RetryOneTime(1)).schemaSet(new SchemaSet(Collections.singletonList(schema), false))
				.build()) {
			schemaClient.start();
			...
			ModeledFramework<TestModel> modeledSchemaClient = ModeledFramework
					.wrap(AsyncCuratorFramework.wrap(schemaClient), modelSpec);
			complete(modeledSchemaClient.set(new TestModel("one", "two", "three", 4, BigInteger.ONE)),
					(dummy, e) -> Assert.assertNull(e));
		}
	}
	...
	public void testVersioned() {
		...
		complete(versioned.set(badVersion),
				(v, e) -> Assert.assertTrue(e instanceof KeeperException.BadVersionException));

		final Stat stat = new Stat();
		complete(client.read(stat));
		// wrong version, needs to fail
		complete(client.delete(stat.getVersion() + 1),
				(v, e) -> Assert.assertTrue(e instanceof KeeperException.BadVersionException));
		// correct version
	...
	public void testAcl() throws NoSuchAlgorithmException {
		...
		try (CuratorFramework authCurator = CuratorFrameworkFactory.builder().connectString(server.getConnectString())
				.retryPolicy(new RetryOneTime(1)).authorization("digest", "test:test".getBytes()).build()) {
			authCurator.start();
