...
/**
 * <p>
 * The JAX-RS web service. Due to how most JAX-RS implementations are written,
 * you must create a concrete class that extends this using your payload type.
 * The concrete class should have the base path that you'd like to use.
 * </p>
 *
 * <p>
 * Because the JAX-RS implementation can create a new instance of the resource
 * for every request, your concrete class must use a context resolver to access
 * the DiscoveryContext. Or, if you are using an IoC framework, you can access
 * it that way.
 * </p>
 *
 * <p>
...
public abstract class DiscoveryResource<T> {
	...
	public Response putService(ServiceInstance<T> instance, @PathParam("name") String name,
	...
	public Response getAny(@PathParam("name") String name) {
		try {
			final List<ServiceInstance<T>> instances = Lists
					.newArrayList(context.getServiceDiscovery().queryForInstances(name));
	...
	private Response internalGet(String name, String id, boolean addDeprecationHeader) {
		try {
			...
			if (addDeprecationHeader) {
				builder = builder.header("Warning",
