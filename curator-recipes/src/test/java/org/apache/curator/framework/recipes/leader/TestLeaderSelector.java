...
public class TestLeaderSelector extends BaseClassForTests {
	...
	public void testErrorPolicies() throws Exception {
		...
		CuratorFramework client = CuratorFrameworkFactory.builder().connectString(server.getConnectString())
				.connectionTimeoutMs(timing.connection()).sessionTimeoutMs(timing.session())
				.retryPolicy(new RetryOneTime(1)).connectionStateErrorPolicy(new StandardConnectionStateErrorPolicy())
				.build();
		try {
	...
	public void testAutoRequeue() throws Exception {
		...
		LeaderSelector selector = null;
		CuratorFramework client = CuratorFrameworkFactory.builder().connectString(server.getConnectString())
				.retryPolicy(new RetryOneTime(1)).sessionTimeoutMs(timing.session()).build();
		try {
			client.start();
	...
	/**
	 * This is similar to
	 * TestLeaderSelector.testKillSessionThenCloseShouldElectNewLeader The
	 * differences are: it restarts the TestingServer instead of killing the session
