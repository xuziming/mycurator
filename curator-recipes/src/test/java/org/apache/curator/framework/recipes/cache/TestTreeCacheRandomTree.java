...
public class TestTreeCacheRandomTree extends BaseTestTreeCache {
	/**
	 * A randomly generated source-of-truth node for
	 * {@link #testGiantRandomDeepTree()}
	 */
	...
	/**
	 * Randomly construct a large tree of test data in memory, mirror it into ZK,
	 * and then use a TreeCache to follow the changes. At each step, assert that
	 * TreeCache matches our source-of-truth test data, and that we see exactly the
	 * set of events we expect to see.
	 */
	private void doTestGiantRandomDeepTree() throws Exception {
		...
		// 320
		// We get more adds than removals because removals only happen if we're at a
		// leaf.
		System.out.println(
	...
	/**
	 * Returns true we should see an event at this path based on maxDepth, false
	 * otherwise.
	 */
