...
public class TestDistributedDelayQueue extends BaseClassForTests {
	...
	public void testSorting() throws Exception {
		...
		client.start();
		try {
			putQueue = QueueBuilder.builder(client, null, new LongSerializer(), "/test2").putInBackground(false)
					.buildDelayQueue();
			putQueue.start();

			Map<Long, Long> data = new HashMap<Long, Long>();

			// Make the earliest a second into the future, so we can ensure that
			// everything's
			// been added prior to the consumption starting. Otherwise it's possible to
			// start
			...
			BlockingQueueConsumer<Long> consumer = new BlockingQueueConsumer<Long>(
					Mockito.mock(ConnectionStateListener.class));
			getQueue = QueueBuilder.builder(client, consumer, new LongSerializer(), "/test2").putInBackground(false)
					.buildDelayQueue();
			getQueue.start();
