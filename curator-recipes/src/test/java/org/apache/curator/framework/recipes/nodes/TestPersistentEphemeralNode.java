...
public class TestPersistentEphemeralNode extends BaseClassForTests {
	...
	public void testListenersReconnectedIsFast() throws Exception {
		...
		try {
			client.start();
			try (PersistentEphemeralNode node = new PersistentEphemeralNode(client,
	...
	public void testNoServerAtStart() throws Exception {
		...
		try {
			client.start();
			node = new PersistentEphemeralNode(client, PersistentEphemeralNode.Mode.EPHEMERAL, "/abc/node",
					"hello".getBytes());
			node.debugWaitMsForBackgroundBeforeClose.set(timing.forSleepingABit().milliseconds());
			node.start();
	...
	public void testRecreatesNodeWhenSessionReconnectsMultipleTimes() throws Exception {
		...
		try {
			...
			// We should be able to disconnect multiple times and each time the node should
			// be recreated.
			for (int i = 0; i < 5; i++) {
				...
				assertTrue(
						stat != null || creationTrigger.firedWithin(timing.forWaiting().seconds(), TimeUnit.SECONDS));
			}
		} finally {
			CloseableUtils.closeQuietly(node);
		}
	...
	public void testRecreatesNodeWhenItGetsDeleted() throws Exception {
		...
		try {
			...
			assertTrue(
					stat != null || createdWatchTrigger.firedWithin(timing.forWaiting().seconds(), TimeUnit.SECONDS));
		} finally {
			CloseableUtils.closeQuietly(node);
		}
	}
	...
	public void testNodesCreateUniquePaths() throws Exception {
		...
		try (PersistentEphemeralNode node1 = new PersistentEphemeralNode(curator,
				PersistentEphemeralNode.Mode.EPHEMERAL_SEQUENTIAL, PATH, new byte[0])) {
			...
			PersistentEphemeralNode node2 = new PersistentEphemeralNode(curator,
					PersistentEphemeralNode.Mode.EPHEMERAL_SEQUENTIAL, PATH, new byte[0]);
	...
	/**
	 * Test that if a persistent ephemeral node is created and the node already
	 * exists that if data is present in the PersistentEphermalNode that it is still
	 * set.
	 * 
	 * @throws Exception
	 */
	...
	/**
	 * See CURATOR-190 For protected nodes on reconnect the current protected name
	 * was passed to the create builder meaning that it got appended to the new
	 * protected node name. This meant that a new node got created on each
	 * reconnect.
	 * 
	 * @throws Exception
	 */
	...
	public void testProtected() throws Exception {
		...
		PersistentEphemeralNode node = new PersistentEphemeralNode(curator,
				PersistentEphemeralNode.Mode.PROTECTED_EPHEMERAL, PATH, new byte[0]);
		try {
			node.debugWaitMsForBackgroundBeforeClose.set(timing.forSleepingABit().milliseconds());
			node.start();
			node.waitForInitialCreate(timing.forWaiting().seconds(), TimeUnit.SECONDS);
	...
	public void testNoWritePermission() throws Exception {
		final ACLProvider aclProvider = new ACLProvider() {
			final ACL acl = new ACL(ZooDefs.Perms.READ | ZooDefs.Perms.CREATE | ZooDefs.Perms.DELETE,
		...
		CuratorFrameworkFactory.Builder builder = CuratorFrameworkFactory.builder();
		CuratorFramework client = builder.connectString(server.getConnectString()).aclProvider(aclProvider)
