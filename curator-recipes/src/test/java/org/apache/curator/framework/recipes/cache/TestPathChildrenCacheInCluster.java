...
public class TestPathChildrenCacheInCluster extends BaseClassForTests {
	...
	public void testMissedDelete() throws Exception {
		...
		try {
			...
			client1 = CuratorFrameworkFactory.newClient(client1Instance.getConnectString(), 1000, 1000,
					new RetryOneTime(1));
			...
			Assert.assertEquals(events.poll(timing.milliseconds(), TimeUnit.MILLISECONDS),
			...
			Assert.assertEquals(events.poll(timing.milliseconds(), TimeUnit.MILLISECONDS),
					PathChildrenCacheEvent.Type.CONNECTION_RECONNECTED);
			Assert.assertEquals(events.poll(timing.milliseconds(), TimeUnit.MILLISECONDS),
					PathChildrenCacheEvent.Type.CHILD_UPDATED); // "/test/node" is different - should register as
																// updated
		} finally {
