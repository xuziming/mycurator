...
public class TestPathChildrenCache extends BaseClassForTests {
	...
	public void testParentContainerMissing() throws Exception {
		...
		try {
			...
			Assert.assertEquals(events.poll(timing.forWaiting().milliseconds(), TimeUnit.MILLISECONDS),
	...
	public void testIssue27Alt() throws Exception {
		...
		try {
			...
			List<PathChildrenCacheEvent.Type> expected = Lists.newArrayList(PathChildrenCacheEvent.Type.CHILD_REMOVED,
					PathChildrenCacheEvent.Type.CHILD_ADDED);
			Assert.assertEquals(expected, events);
		} finally {
			CloseableUtils.closeQuietly(cache);
	...
	public void testKilledSession() throws Exception {
		...
		try {
			client = CuratorFrameworkFactory.newClient(server.getConnectString(), timing.session(), timing.connection(),
					new RetryOneTime(1));
			client.start();
			client.create().forPath("/test");
	...
	/**
	 * Tests the case where there's an outstanding operation being executed when the
	 * cache is shut down. See CURATOR-121, this was causing misleading warning
	...
	public void testInterruptedOperationOnShutdown() throws Exception {
		CuratorFramework client = CuratorFrameworkFactory.newClient(server.getConnectString(), 30000, 30000,
				new RetryOneTime(1));
		client.start();
