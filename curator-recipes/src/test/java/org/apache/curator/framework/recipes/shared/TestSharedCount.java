...
public class TestSharedCount extends CuratorTestBase {
	...
	public void testDisconnectEventOnWatcherDoesNotRetry() throws Exception {
		...
		CuratorFramework curatorFramework = CuratorFrameworkFactory.newClient(server.getConnectString(),
	...
	public void testDisconnectReconnectWithMultipleClients() throws Exception {
		Timing timing = new Timing();
		CuratorFramework curatorFramework1 = CuratorFrameworkFactory.newClient(server.getConnectString(),
				new RetryNTimes(10, 500));
		CuratorFramework curatorFramework2 = CuratorFrameworkFactory.newClient(server.getConnectString(),
				new RetryNTimes(10, 500));
		...
		SharedCount sharedCountWithFaultyWatcher = createSharedCountWithFaultyWatcher(curatorFramework2,
		...
		try {
			...
			Assert.assertEquals(
					listener1.gotChangeEvent.awaitAdvanceInterruptibly(0, timing.seconds(), TimeUnit.SECONDS), 1);
			...
			for (int i = 0; i < 10; i++) {
				...
				Assert.assertEquals(listener2.getReconnectEvent.awaitAdvanceInterruptibly(i,
						timing.forWaiting().seconds(), TimeUnit.SECONDS), i + 1);
				// CURATOR-311 introduces to Curator's client reading server's shared count
				// value
				// when client's state gets ConnectionState.RECONNECTED. Following tests ensures
				// that.
				Assert.assertEquals(listener2.gotChangeEvent.awaitAdvanceInterruptibly(i, timing.forWaiting().seconds(),
