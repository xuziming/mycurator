...
public class TestBoundedDistributedQueue extends BaseClassForTests {
		...
		try {
			...
			executor.submit(new Callable<Void>() {
				...
				public Void call() throws Exception {
					...
					while (!Thread.currentThread().isInterrupted() && client.getState() == CuratorFrameworkState.STARTED
							&& !isDone.get()) {
						synchronized (lock) {
	...
	public void testSimple() throws Exception {
		...
		try {
			...
			Assert.assertTrue(queue.put("1", timing.milliseconds(), TimeUnit.MILLISECONDS)); // should end up in
																								// consumer
			Assert.assertTrue(queue.put("2", timing.milliseconds(), TimeUnit.MILLISECONDS)); // should sit blocking in
																								// DistributedQueue
