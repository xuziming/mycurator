...
public class TestDistributedPriorityQueue extends BaseClassForTests {
	...
	public void testMinItemsBeforeRefresh() throws Exception {
		...
		try {
			...
			Assert.assertTrue(Math.abs(minItemsBeforeRefresh - count) < minItemsBeforeRefresh, String
					.format("Diff: %d - min: %d", Math.abs(minItemsBeforeRefresh - count), minItemsBeforeRefresh)); // allows
																													// for
																													// some
																													// slack
																													// -
	...
	public void testSortingWhileTaking() throws Exception {
		...
		try {
			...
			queue.put(1000, 1); // lower priority
			timing.sleepABit();
			Assert.assertEquals(blockingQueue.poll(timing.seconds(), TimeUnit.SECONDS), new Integer(1)); // is in
																											// consumer
																											// already
	...
	public void testSimple() throws Exception {
		...
		try {
			...
			queue.put(Integer.MIN_VALUE, Integer.MIN_VALUE); // the queue background thread will be blocking with the
																// first item - make sure it's the lowest value
			...
			while (queue.getCache().getData().children.size() < (nums.size() - 1)) // -1 because the first message has
