...
public class TestInterProcessSemaphore extends BaseClassForTests {
	...
	public void testAcquireAfterLostServer() throws Exception {
		...
		final CuratorFramework client = CuratorFrameworkFactory.newClient(server.getConnectString(),
				timing.forWaiting().milliseconds(), timing.connection(), new RetryOneTime(1)); // long session time on
																								// purpose
		try {
			client.start();

			InterProcessSemaphoreV2.debugAcquireLatch = new CountDownLatch(1); // cause one of the semaphores to create
																				// its node and then wait
			InterProcessSemaphoreV2.debugFailedGetChildrenLatch = new CountDownLatch(1); // semaphore will notify when
																							// getChildren() fails
			...
			Runnable runner = new Runnable() {
				...
				public void run() {
					while (!Thread.currentThread().isInterrupted()) {
						InterProcessSemaphoreV2 semaphore = new InterProcessSemaphoreV2(client, SEMAPHORE_PATH,
								MAX_SEMAPHORES);
						Lease lease = null;
						try {
			...
			InterProcessSemaphoreV2.debugAcquireLatch.countDown(); // the waiting semaphore proceeds to getChildren -
																	// which should fail
			Assert.assertTrue(timing.awaitLatch(InterProcessSemaphoreV2.debugFailedGetChildrenLatch)); // wait until
																										// getChildren
																										// fails
	...
	public void testClientClose() throws Exception {
		...
		try {
			client1 = CuratorFrameworkFactory.newClient(server.getConnectString(), timing.session(),
					timing.connection(), new RetryOneTime(1));
			client2 = CuratorFrameworkFactory.newClient(server.getConnectString(), timing.session(),
					timing.connection(), new RetryOneTime(1));
				...
				public Object call() throws Exception {
					...
					try {
						...
						for (int i = 0; i < LOOP_QTY; ++i) {
							...
							Collection<Lease> leases = semaphore.acquire(thisQty, timing.forWaiting().seconds(),
									TimeUnit.SECONDS);
	...
	public void testChildReaperCleansUpLockNodes() throws Exception {
		...
		try {
			...
			childReaper = new ChildReaper(client, "/test", Reaper.Mode.REAP_UNTIL_GONE,
					ChildReaper.newExecutorService(), 1, "/test-leader", InterProcessSemaphoreV2.LOCK_SCHEMA);
			childReaper.start();
			...
			try {
				...
				// this is OK - if Container Nodes are used the "/test" path will go away - no
				// point in updating the test for deprecated code
			}
		} finally {
	...
	public void testInterruptAcquire() throws Exception {
		...
		try {
			...
			// Wait until second lease is created and the wait is started for it to become
			// active
			...
			// Assert that after closing the first (current) semaphore, we can acquire a new
			// one
