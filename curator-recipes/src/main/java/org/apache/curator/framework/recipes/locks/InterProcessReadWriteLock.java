 ...
 * the style of a re-entrant lock. Non-re-entrant readers are not allowed until
 * all write locks held by the writing thread/process have been released.
 * Additionally, a writer can acquire the read lock, but not vice-versa. If a
 * reader tries to acquire the write lock it will never succeed.<br>
 * <br>
 *
 * <b>Lock downgrading</b><br>
 * Re-entrancy also allows downgrading from the write lock to a read lock, by
...
public class InterProcessReadWriteLock {
	...
	public InterProcessReadWriteLock(CuratorFramework client, String basePath, byte[] lockData) {
		...
		writeMutex = new InternalInterProcessMutex(client, basePath, WRITE_LOCK_NAME, lockData, 1,
		...
		readMutex = new InternalInterProcessMutex(client, basePath, READ_LOCK_NAME, lockData, Integer.MAX_VALUE,
				new SortingLockInternalsDriver() {
					@Override
					public PredicateResults getsTheLock(CuratorFramework client, List<String> children,
							String sequenceNodeName, int maxLeases) throws Exception {
