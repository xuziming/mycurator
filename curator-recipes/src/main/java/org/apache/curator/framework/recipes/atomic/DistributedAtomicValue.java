...
/**
 * <p>
 * A distributed value that attempts atomic sets. It first tries uses optimistic
 * locking. If that fails, an optional {@link InterProcessMutex} is taken. For
 * both optimistic and mutex, a retry policy is used to retry the increment.
 * </p>
 *
 * <p>
 * The various methods return an {@link AtomicValue} object. You must
 * <b>always</b> check {@link AtomicValue#succeeded()}. None of the methods
 * (other than get()) are guaranteed to succeed.
 * </p>
 */
public class DistributedAtomicValue {
	...
	/**
	 * Creates in mutex promotion mode. The optimistic lock will be tried first
	 * using the given retry policy. If the increment does not succeed, a
	 * {@link InterProcessMutex} will be tried with its own retry policy
	 *
	 * @param client         the client
	 * @param path           path to hold the value
	 * @param retryPolicy    the retry policy to use
	...
	/**
	 * Returns the current value of the counter. NOTE: if the value has never been
	 * set, <code>0</code> is returned.
	 *
	 * @return value info
	 ...
	 * {@code ==} the expected value. Remember to always check
	 * {@link AtomicValue#succeeded()}.
	 *
	 *
	...
	/**
	 * Atomic values are initially set to the equivalent of <code>NULL</code> in a
	 * database. Use this method to initialize the value. The value will be set if
	 * and only iff the node does not exist.
	 *
			...
			try {
				...
				while (!done) {
					...
					if (tryOnce(result, makeValue)) {
						...
						if (!promotedToLock.getRetryPolicy().allowRetry(retryCount++,
								System.currentTimeMillis() - startMs, RetryLoop.getDefaultRetrySleeper())) {
							done = true;
						}
					}
				}
