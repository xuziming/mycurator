...
/**
 * Utility to clean up parent lock nodes so that they don't stay around as
 * garbage
 *
 * @deprecated Since 2.9.0 - Reaper/ChildReaper are no longer needed. Use
 *             {@link CreateMode#CONTAINER}. Also, all Curator recipes create
 *             container parents.
 */
@Deprecated
public class Reaper implements Closeable {
	...
	/**
	 * Uses the default reaping threshold of 5 minutes and creates an internal
	 * thread pool
	 *
	 * @param client client
	 */
	 ...
	 *
	 * @param client             client
	 * @param reapingThresholdMs threshold in milliseconds that determines that a
	 *                           path can be deleted
	 */
	 ...
	 * @param reapingThresholdMs threshold in milliseconds that determines that a
	 *                           path can be deleted
	 * @param leaderLatch        a pre-created leader latch to ensure only 1 reaper
	 ...
	 * @param leaderLatch        a pre-created leader latch to ensure only 1 reaper
	 *                           is active in the cluster
	 * @param ownsLeaderLatch    indicates whether or not the reaper owns the leader
	 *                           latch (if it exists) and thus should start/stop it
	 */
	private Reaper(CuratorFramework client, ScheduledExecutorService executor, int reapingThresholdMs,
			LeaderLatch leaderLatch, boolean ownsLeaderLatch) {
		this.client = client;
	...
	/**
	 * Add a path (using Mode.REAP_INDEFINITELY) to be checked by the reaper. The
	 * path will be checked periodically until the reaper is closed.
	 *
