...
/**
 * A version of {@link DistributedQueue} that allows IDs to be associated with
 * queue items. Items can then be removed from the queue if needed
 */
...
public class DistributedIdQueue<T> implements QueueBase<T> {
	...
	DistributedIdQueue(CuratorFramework client, QueueConsumer<T> consumer, QueueSerializer<T> serializer,
		...
		queue = new DistributedQueue<T>(client, consumer, serializer, queuePath, threadFactory, executor,
				minItemsBeforeRefresh, refreshOnWatch, lockPath, maxItems, putInBackground, finalFlushMs) {
			@Override
	...
	/**
	 * Put an item into the queue with the given Id<br>
	 * <br>
	...
	/**
	 * Same as {@link #put(Object, String)} but allows a maximum wait time if an
	...
	private Parts parseId(String name) {
		...
		return new Parts(name.substring(firstIndex + 1, secondIndex),
