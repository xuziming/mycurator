...
public class ChildReaper implements Closeable {
	 ...
	 *                           path that must be reaped in addition to the child
	 *                           nodes
	 */
	public ChildReaper(CuratorFramework client, String path, Reaper.Mode mode, ScheduledExecutorService executor,
			int reapingThresholdMs, String leaderPath, Set<String> lockSchema) {
		this.client = client;
		this.mode = mode;
	...
	/**
	 * If a node has so many children that {@link CuratorFramework#getChildren()}
	 * will fail (due to jute.maxbuffer) it can cause connection instability. Set
	 * the max number of children here to prevent the path from being queried in
	 * these cases. The number should usually be: average-node-name-length/1000000
	 *
