 ...
 * When items are added to the queue, a delay value is given. The item will not
 * be sent to a consumer until the time elapses.
 * </p>
 */
...
public class DistributedDelayQueue<T> implements Closeable, QueueBase<T> {
	...
	DistributedDelayQueue(CuratorFramework client, QueueConsumer<T> consumer, QueueSerializer<T> serializer,
	 ...
	 * @param delayUntilEpoch future epoch (milliseconds) when this item will be
	...
	/**
	 * Same as {@link #put(Object, long)} but allows a maximum wait time if an upper
	...
	/**
	 * Same as {@link #putMulti(MultiItem, long)} but allows a maximum wait time if
