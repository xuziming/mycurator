...
public class PersistentNode implements Closeable {
	...
	private final AtomicReference<CreateModable<ACLBackgroundPathAndBytesable<String>>> createMethod = new AtomicReference<CreateModable<ACLBackgroundPathAndBytesable<String>>>(
			null);
	...
	private final BackgroundCallback setDataCallback = new BackgroundCallback() {
		...
		@Override
		public void processResult(CuratorFramework dummy, CuratorEvent event) throws Exception {
			// If the result is ok then initialisation is complete (if we're still
			// initialising)
			// Don't retry on other errors as the only recoverable cases will be connection
			// loss
			// and the node not existing, both of which are already handled by other
			// watches.
	 ...
	 */
	public PersistentNode(CuratorFramework givenClient, final CreateMode mode, boolean useProtection,
			final String basePath, byte[] initData, long ttl) {
		this.useProtection = useProtection;
		this.client = Preconditions.checkNotNull(givenClient, "client cannot be null")
	...
	/**
	 * You must call start() to initiate the persistent node. An attempt to create
	 * the node in the background will be started
	 */
	public void start() {
	...
	public void setData(byte[] data) throws Exception {
		...
		Preconditions.checkState(nodePath.get() != null,
	...
	private void createNode() {
		...
		try {
			...
			if (localCreateMethod == null) {
				CreateBuilderMain createBuilder = SafeIsTtlMode.isTtl(mode) ? client.create().withTtl(ttl)
						: client.create();
				CreateModable<ACLBackgroundPathAndBytesable<String>> tempCreateMethod = useProtection
			...
			localCreateMethod.withMode(getCreateMode(existingPath != null)).inBackground(backgroundCallback)
					.forPath(createPath, data.get());
		} catch (Exception e) {
			ThreadUtils.checkInterrupted(e);
			throw new RuntimeException("Creating node. BasePath: " + basePath, e); // should never happen unless there's
																					// a programming error - so throw
																					// RuntimeException
		}
	}
	...
	private void notifyListeners() {
		...
		listeners.forEach(new Function<PersistentNodeListener, Void>() {
			@Override
