 ...
 * contends for leadership, one will be assigned leader until it releases
 * leadership at which time another one from the group will be chosen.
 * </p>
 * <p>
 * Note that this class uses an underlying {@link InterProcessMutex} and as a
 * result leader election is "fair" - each user will become leader in the order
 * originally requested (from ZK's point of view).
 * </p>
 */
public class LeaderSelector implements Closeable {
	...
	public LeaderSelector(CuratorFramework client, String leaderPath, LeaderSelectorListener listener) {
		this(client, leaderPath,
				new CloseableExecutorService(Executors.newSingleThreadExecutor(defaultThreadFactory), true), listener);
	}
	 ...
	 * @param listener      listener
	 * @deprecated This constructor was poorly thought out. Custom executor is
	 *             useless. Use this version instead:
	 *             {@link #LeaderSelector(CuratorFramework, String, ExecutorService, LeaderSelectorListener)}
	 */
	@Deprecated
	public LeaderSelector(CuratorFramework client, String leaderPath, ThreadFactory threadFactory, Executor executor,
	 ...
	 * @param listener        listener
	 */
	public LeaderSelector(CuratorFramework client, String leaderPath, CloseableExecutorService executorService,
			LeaderSelectorListener listener) {
	...
	/**
	 * By default, when
	 * {@link LeaderSelectorListener#takeLeadership(CuratorFramework)} returns, this
	 * instance is not requeued. Calling this method puts the leader selector into a
	 * mode where it will always requeue itself.
	 */
	...
	/**
	 * Sets the ID to store for this leader. Will be the value returned when
	 * {@link #getParticipants()} is called. IMPORTANT: must be called prior to
	 * {@link #start()} to have effect.
	 *
	 * @param id ID
	 */
	...
	/**
	 * Attempt leadership. This attempt is done in the background - i.e. this method
	 * returns immediately.<br>
	 * <br>
	 * <b>IMPORTANT: </b> previous versions allowed this method to be called
	 * multiple times. This is no longer supported. Use {@link #requeue()} for this
	 * purpose.
	 */
	public void start() {
	 ...
	 * nothing happens and false is returned. If the instance was not queued, it is
	 * re-qeued and true is returned
	 *
	...
	/**
	 * <p>
	 * Return the id for the current leader. If for some reason there is no current
	 * leader, a dummy participant is returned.
	 * </p>
	 * <p>
	 * <p>
	 * <B>NOTE</B> - this method polls the ZK server. Therefore it can possibly
	...
	void doWork() throws Exception {
		...
		try {
			...
			if (hasLeadership) {
				hasLeadership = false;
				boolean wasInterrupted = Thread.interrupted(); // clear any interrupted tatus so that mutex.release()
																// works immediately
				try {
					mutex.release();
				} catch (Exception e) {
	...
	private void doWorkLoop() throws Exception {
		...
		if ((exception != null) && !autoRequeue.get()) // autoRequeue should ignore connection loss or session expired
