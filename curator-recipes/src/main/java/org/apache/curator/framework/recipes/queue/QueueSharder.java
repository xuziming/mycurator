 ...
 *
 * <p>
 * Due to limitations in ZooKeeper's transport layer, a single queue will break
 * if it has more than 10K-ish items in it. This class provides a facade over
 * multiple distributed queues. It monitors the queues and if any one of them
 * goes over a threshold, a new queue is added. Puts are distributed amongst the
 * queues.
 * </p>
 *
 * <p>
 * NOTE: item ordering is maintained within each managed queue but cannot be
 * maintained across queues. i.e. items might get consumed out of order if they
 * are in different managed queues.
 * </p>
 */
public class QueueSharder<U, T extends QueueBase<U>> implements Closeable {
	 ...
	 * @param leaderPath     path for the leader that monitors queue sizes (must be
	 *                       different than queuePath)
	 ...
	 * It should be considered a random managed queue.
	 *
	 * @return a queue
	 */
	public T getQueue() {
		...
		try {
			...
			if (addAQueueIfLeader && leaderLatch.hasLeadership()) {
				if (queues.size() < policies.getMaxQueues()) {
					log.info(String.format("Adding a queue due to exceeded threshold. Queue Size: %d - Threshold: %d",
