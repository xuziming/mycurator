...
/**
 * Utility class to allow execution of logic once a ZooKeeper connection becomes
 * available.
 */
public class AfterConnectionEstablished {
	...
	/**
	 * Spawns a new new background thread that will block until a connection is
	 * available and then execute the 'runAfterConnection' logic
	 *
	...
	public static Future<?> execute(final CuratorFramework client, final Runnable runAfterConnection) throws Exception {
		...
		final ExecutorService executor = ThreadUtils
				.newSingleThreadExecutor(ThreadUtils.getProcessName(runAfterConnection.getClass()));
