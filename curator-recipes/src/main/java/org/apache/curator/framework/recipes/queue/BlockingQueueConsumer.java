...
public class BlockingQueueConsumer<T> implements QueueConsumer<T> {
	...
	/**
	 * Take the next item from the queue, waiting up to the specified time for an
	 * available item. If the time elapses, <code>null</code> is returned.
	 *
	 ...
	 * queue. A failure encountered while attempting to add elements to collection
	 * <tt>c</tt> may result in elements being in neither, either or both
	 * collections when the associated exception is thrown. Attempts to drain a
	 * queue to itself result in <tt>IllegalArgumentException</tt>. Further, the
