...
public class PathChildrenCache implements Closeable {
	 ...
	 * @deprecated use {@link #PathChildrenCache(CuratorFramework, String, boolean)}
	 *             instead
	 */
	@Deprecated
	 ...
	 * @deprecated use
	 *             {@link #PathChildrenCache(CuratorFramework, String, boolean, ThreadFactory)}
	 *             instead
	 */
	@Deprecated
	...
	public PathChildrenCache(CuratorFramework client, String path, boolean cacheData, boolean dataIsCompressed,
		...
		this(client, path, cacheData, dataIsCompressed,
				new CloseableExecutorService(Executors.newSingleThreadExecutor(threadFactory), true));
	}
	 ...
	 *                         stat
	 * @param dataIsCompressed if true, data in the path is compressed
	 * @param executorService  ExecutorService to use for the PathChildrenCache's
	 *                         background thread. This service should be single
	 *                         threaded, otherwise the cache may see inconsistent
	 *                         results.
	 */
	public PathChildrenCache(CuratorFramework client, String path, boolean cacheData, boolean dataIsCompressed,
	 ...
	 *                         PathChildrenCache's background thread. This service
	 *                         should be single threaded, otherwise the cache may
	...
	/**
	 * Start the cache. The cache is not started automatically. You must call this
	 * method.
	 *
	 ...
	 *
	 * @param buildInitial if true, {@link #rebuild()} will be called before this
	 *                     method returns in order to get an initial view of the
	 *                     node; otherwise, the cache will be initialized
	 *                     asynchronously
	 * @throws Exception errors
	...
	public enum StartMode {
		/**
		 * The cache will be primed (in the background) with initial values. Events for
		 * existing and new nodes will be posted.
		 */
		NORMAL,

		/**
		 * The cache will be primed (in the foreground) with initial values.
		 * {@link PathChildrenCache#rebuild()} will be called before the
		 * {@link PathChildrenCache#start(StartMode)} method returns in order to get an
		 * initial view of the node.
		 */
		BUILD_INITIAL_CACHE,
	...
	public void rebuild() throws Exception {
		...
		// this is necessary so that any updates that occurred while rebuilding are
		// taken
		offerOperation(new RefreshOperation(this, RefreshMode.FORCE_GET_DATA_AND_STAT));
	}

	/**
	 * NOTE: this is a BLOCKING method. Rebuild the internal cache for the given
	 * node by querying for all needed data WITHOUT generating any events to send to
	 * listeners.
	 *
	...
	/**
	 * Return the current data. There are no guarantees of accuracy. This is merely
	 * the most recent view of the data. The data is returned in sorted order.
	 *
	 ...
	 * accuracy. This is merely the most recent view of the data. If there is no
	 * child with that path, <code>null</code> is returned.
	 *
	 ...
	 * Subsequent calls to {@link ChildData#getData()} for this node will return
	 * <code>null</code>.
	 *
	 ...
	 *
	 * @param fullPath  the path of the node to clear
	 * @param ifVersion if non-negative, only clear the data if the data's version
	...
	/**
	 * Clears the current data without beginning a new query and without generating
	 * any events for listeners.
	 */
	public void clear() {
		currentData.clear();
	}
	...
	enum RefreshMode {
		STANDARD, FORCE_GET_DATA_AND_STAT, POST_INITIALIZED, NO_NODE_EXCEPTION
	}
	...
	void getDataAndStat(final String fullPath) throws Exception {
		...
		if (USE_EXISTS && !cacheData) {
			...
			// always use getData() instead of exists() to avoid leaving unneeded watchers
			// which is a type of resource leak
	...
	protected void remove(String fullPath) {
		...
		if (data != null) {
			offerOperation(new EventOperation(this,
					new PathChildrenCacheEvent(PathChildrenCacheEvent.Type.CHILD_REMOVED, data)));
		}
	...
	private void handleStateChange(ConnectionState newState) {
		...
		case SUSPENDED: {
			offerOperation(new EventOperation(this,
					new PathChildrenCacheEvent(PathChildrenCacheEvent.Type.CONNECTION_SUSPENDED, null)));
			break;
		}
	...
	private void applyNewData(String fullPath, int resultCode, Stat stat, byte[] bytes) {
		if (resultCode == KeeperException.Code.OK.intValue()) // otherwise - node must have dropped or something - we
																// should be getting another event
		{
			...
			if (previousData == null) // i.e. new
			{
				...
				offerOperation(new EventOperation(this,
						new PathChildrenCacheEvent(PathChildrenCacheEvent.Type.CHILD_UPDATED, data)));
			}
	...
	private void maybeOfferInitializedEvent(Map<String, ChildData> localInitialSet) {
		if (!hasUninitialized(localInitialSet)) {
			...
			if (initialSet.getAndSet(null) != null) // avoid edge case - don't send more than 1 INITIALIZED event
			{
				...
				PathChildrenCacheEvent event = new PathChildrenCacheEvent(PathChildrenCacheEvent.Type.INITIALIZED,
						null) {
					@Override
	 ...
	 * <p>
	 * This method is synchronized because it has to check state about whether this
	 * instance is still open. Without this check there is a race condition with the
	 * dataWatchers that get set. Even after this object is closed() it can still be
	 * called by those watchers, because the close() method cannot actually disable
	 * the watcher.
	 * <p>
	 * The synchronization overhead should be minimal if non-existant as this is
	 * generally only called from the ZK client thread and will only contend if
	 * close() is called in parallel with an update, and that's the exact state we
