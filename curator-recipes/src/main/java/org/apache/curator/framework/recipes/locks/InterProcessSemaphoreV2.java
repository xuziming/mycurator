...
public class InterProcessSemaphoreV2 {
		...
		if (count != null) {
			count.addListener(new SharedCountListener() {
				...
				public void stateChanged(CuratorFramework client, ConnectionState newState) {
					// no need to handle this here - clients should set their own connection state
					// listener
				}
			});
		}
			...
			while (qty-- > 0) {
				...
				while (!isDone) {
					...
					case RETRY_DUE_TO_MISSING_NODE: {
						...
						// this can happen when the session expires, etc. So, if the retry allows, just
						// try it all again
	...
	private InternalAcquireResult internalAcquire1Lease(ImmutableList.Builder<Lease> builder, long startMs,
		...
		try {
			PathAndBytesable<String> createBuilder = client.create().creatingParentContainersIfNeeded().withProtection()
