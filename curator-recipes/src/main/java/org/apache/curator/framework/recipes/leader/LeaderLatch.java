...
public class LeaderLatch implements Closeable {
	...
	/**
	 * Remove this instance from the leadership election. If this instance is the
	 * leader, leadership is released. IMPORTANT: the only way to release leadership
	 * is by calling close(). All LeaderLatch instances must eventually be closed.
	 *
	 ...
	 * is by calling close(). All LeaderLatch instances must eventually be closed.
	 *
	 * @param closeMode allows the default close mode to be overridden at the time
	 *                  the latch is closed.
	 * @throws IOException errors
	 */
	 ...
	 * common to pass in a single-threaded executor so that you can be certain that
	 * listener methods are called in sequence, but if you are fine with them being
	 * called out of order you are welcome to use multiple threads.
	 * </p>
	 *
	 ...
	 * </p>
	 * <p>
	 * Otherwise the current thread becomes disabled for thread scheduling purposes
	 ...
	 * <p>
	 * Causes the current thread to wait until this instance acquires leadership
	 * unless the thread is {@linkplain Thread#interrupt interrupted}, the specified
	 * waiting time elapses or the instance is {@linkplain #close() closed}.
	 * </p>
	 * <p>
	 * </p>
	 * <p>
	 ...
	 * <p>
	 * Otherwise the current thread becomes disabled for thread scheduling purposes
	 * and lies dormant until one of four things happen:
	 * </p>
	 * <ul>
	 * <li>This instance becomes the leader</li>
	 * <li>Some other thread {@linkplain Thread#interrupt interrupts} the current
	 ...
	 * </p>
	 * <p>
	 * If the specified waiting time elapses or the instance is {@linkplain #close()
	 * closed} then the value {@code false} is returned. If the time is less than or
	 ...
	 *         waiting time elapsed before the count reached zero or the instances
	 *         was closed
	 * @throws InterruptedException if the current thread is interrupted while
	 *                              waiting
	 */
	...
	/**
	 * Returns this instances current state, this is the only way to verify that the
	 * object has been closed before closing again. If you try to close a latch
	 * multiple times, the close() method will throw an IllegalArgumentException
	 * which is often not caught and ignored (CloseableUtils.closeQuietly() only
	 * looks for IOException).
	 *
	...
	private void checkLeadership(List<String> children) throws Exception {
		...
		if (ourIndex < 0) {
			...
			// use getData() instead of exists() to avoid leaving unneeded watchers which is
			// a type of resource leak
