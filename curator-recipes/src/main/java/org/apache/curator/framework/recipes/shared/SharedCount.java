...
public class SharedCount implements Closeable, SharedCountReader, Listenable<SharedCountListener> {
	 ...
	 * @param seedValue the initial value for the count if/f the path has not yet
	 *                  been created
	 */
	...
	/**
	 * Changes the shared count only if its value has not changed since this client
	 * last read it. If the count has changed, the value is not set and this
	 * client's view of the value is updated. i.e. if the count is not successful
	 * you can get the updated value by calling {@link #getCount()}.
	 *
	 * @deprecated use {@link #trySetCount(VersionedValue, int)} for stronger
	...
	/**
	 * Changes the shared count only if its value has not changed since the version
	 * specified by newCount. If the count has changed, the value is not set and
	 * this client's view of the value is updated. i.e. if the count is not
	 * successful you can get the updated value by calling {@link #getCount()}.
	 *
	 * @param newCount the new value to attempt
	 * @return true if the change attempt was successful, false if not. If the
	 *         change was not successful, {@link #getCount()} will return the
	...
	public boolean trySetCount(VersionedValue<Integer> previous, int newCount) throws Exception {
		VersionedValue<byte[]> previousCopy = new VersionedValue<byte[]>(previous.getVersion(),
	...
	/**
	 * The shared count must be started before it can be used. Call {@link #close()}
	 * when you are finished with the shared count
	 *
	 * @throws Exception ZK errors, interruptions, etc.
	 */
