 ...
 * </p>
 * <p>
 * Internally, this uses a {@link DistributedQueue}. The only difference is that
 * you specify a priority when putting into the queue.
 * </p>
 * <p>
 * IMPORTANT NOTE: The priority queue will perform far worse than a standard
 * queue. Every time an item is added to/removed from the queue, every watcher
 * must re-get all the nodes
 * </p>
 */
...
public class DistributedPriorityQueue<T> implements Closeable, QueueBase<T> {
	...
	DistributedPriorityQueue(CuratorFramework client, QueueConsumer<T> consumer, QueueSerializer<T> serializer,
			String queuePath, ThreadFactory threadFactory, Executor executor, int minItemsBeforeRefresh,
			String lockPath, int maxItems, boolean putInBackground, int finalFlushMs) {
		Preconditions.checkArgument(minItemsBeforeRefresh >= 0, "minItemsBeforeRefresh cannot be negative");

		queue = new DistributedQueue<T>(client, consumer, serializer, queuePath, threadFactory, executor,
				minItemsBeforeRefresh, true, lockPath, maxItems, putInBackground, finalFlushMs);
	}
	...
	/**
	 * Same as {@link #put(Object, int)} but allows a maximum wait time if an upper
	 * bound was set via {@link QueueBuilder#maxItems}.
	 *
	 * @param item     item to add
	 * @param priority item's priority - lower numbers come out of the queue first
	...
	/**
	 * Add a set of items with the same priority into the queue. Adding is done in
	 * the background - thus, this method will return quickly.<br>
	 * <br>
	 * NOTE: if an upper bound was set via {@link QueueBuilder#maxItems}, this
	 * method will block until there is available space in the queue.
	 *
	...
	/**
	 * Same as {@link #putMulti(MultiItem, int)} but allows a maximum wait time if
	 * an upper bound was set via {@link QueueBuilder#maxItems}.
	 *
	 * @param items    items to add
	 * @param priority item priority - lower numbers come out of the queue first
	 * @param maxWait  maximum wait
