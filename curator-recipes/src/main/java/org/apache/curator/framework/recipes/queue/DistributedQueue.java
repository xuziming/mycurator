...
/**
 * <p>
 * An implementation of the Distributed Queue ZK recipe. Items put into the
 * queue are guaranteed to be ordered (by means of ZK's PERSISTENT_SEQUENTIAL
 * node).
 * </p>
 *
 * <p>
 ...
 * </p>
 * <ul>
 * <li>If a single consumer takes items out of the queue, they will be ordered
 * FIFO. i.e. if ordering is important, use a {@link LeaderSelector} to nominate
 * a single consumer.</li>
 * <li>Unless a {@link QueueBuilder#lockPath(String)} is used, there is only
 * guaranteed processing of each message to the point of receipt by a given
 * instance.
 * <li>If an instance receives an item from the queue but dies while processing
 * it, the item will be lost. If you need message recoverability, use a
 * {@link QueueBuilder#lockPath(String)}</li>
 * </ul>
 */
...
public class DistributedQueue<T> implements QueueBase<T> {
	...
	DistributedQueue(CuratorFramework client, QueueConsumer<T> consumer, QueueSerializer<T> serializer,
			String queuePath, ThreadFactory threadFactory, Executor executor, int minItemsBeforeRefresh,
			boolean refreshOnWatch, String lockPath, int maxItems, boolean putInBackground, int finalFlushMs) {
	...
	/**
	 * Used when the queue is created with a {@link QueueBuilder#lockPath(String)}.
	 * Determines the behavior when the queue consumer throws an exception
	 *
	 * @param newErrorMode the new error mode (the default is
	 *                     {@link ErrorMode#REQUEUE}
	 */
	@Override
	...
	/**
	 * Add an item into the queue. Adding is done in the background - thus, this
	 * method will return quickly.<br>
	 * <br>
	 * NOTE: if an upper bound was set via {@link QueueBuilder#maxItems}, this
	 * method will block until there is available space in the queue.
	 *
	...
	/**
	 * Same as {@link #put(Object)} but allows a maximum wait time if an upper bound
	 * was set via {@link QueueBuilder#maxItems}.
	 *
	 * @param item    item to add
	...
	/**
	 * Add a set of items into the queue. Adding is done in the background - thus,
	 * this method will return quickly.<br>
	 * <br>
	 * NOTE: if an upper bound was set via {@link QueueBuilder#maxItems}, this
	 * method will block until there is available space in the queue.
	 *
	...
	/**
	 * Same as {@link #putMulti(MultiItem)} but allows a maximum wait time if an
	 * upper bound was set via {@link QueueBuilder#maxItems}.
	 *
	 * @param items   items to add
	 * @param maxWait maximum wait
	 * @param unit    wait unit
	...
	/**
	 * Return the most recent message count from the queue. This is useful for
	 * debugging/information purposes only.
	 *
	...
	protected boolean processWithLockSafety(String itemNode, ProcessType type) throws Exception {
		...
		try {
			...
			if (requeue) {
				client.inTransaction().delete().forPath(itemPath).and().create()
						.withMode(CreateMode.PERSISTENT_SEQUENTIAL).forPath(makeRequeueItemPath(itemPath), bytes).and()
						.commit();
			} else {
