...
public class TreeCache implements Closeable {
	...
	public static final class Builder {
		...
		/**
		 * Sets the maximum depth to explore/watch. A {@code maxDepth} of {@code 0} will
		 * watch only the root node (like {@link NodeCache}); a {@code maxDepth} of
		 * {@code 1} will watch the root node and its immediate children (kind of like
		 * {@link PathChildrenCache}. Default: {@code Integer.MAX_VALUE}
		 */
		 ...
		 * Change this behavior with this method. NOTE: parent nodes are created as
		 * containers
		 *
		 ...
		 * for every created path. Change this behavior with this method.
		 * 
		...
		/**
		 * By default, {@link DefaultTreeCacheSelector} is used. Change the selector
		 * here.
		 *
	...
	/**
	 * Create a TreeCache builder for the given client and path to configure
	 * advanced options.
	 * <p/>
	 ...
	 * @param path   the path to the root node to watch/explore; this path need not
	...
	private static final AtomicReferenceFieldUpdater<TreeNode, ChildData> childDataUpdater = AtomicReferenceFieldUpdater
			.newUpdater(TreeNode.class, ChildData.class, "childData");

	@SuppressWarnings({ "unchecked", "rawtypes" })
	private static final AtomicReferenceFieldUpdater<TreeNode, ConcurrentMap<String, TreeNode>> childrenUpdater = (AtomicReferenceFieldUpdater) AtomicReferenceFieldUpdater
			.newUpdater(TreeNode.class, ConcurrentMap.class, "children");
			...
			case CHILDREN:
				if (event.getResultCode() == KeeperException.Code.OK.intValue()) {
					...
					if (isLive(oldChildData) && oldChildData.getStat().getMzxid() == newStat.getMzxid()) {
						...
						childDataUpdater.compareAndSet(this, oldChildData,
				...
				if (event.getResultCode() == KeeperException.Code.OK.intValue()) {
					...
					while (true) {
						...
						if (childDataUpdater.compareAndSet(this, oldChildData, toUpdate)) {
							publishEvent(isLive(oldChildData) ? TreeCacheEvent.Type.NODE_UPDATED
									: TreeCacheEvent.Type.NODE_ADDED, toPublish);
							break;
						}
					}
				} else if (event.getResultCode() == KeeperException.Code.NONODE.intValue()) {
	...
	public TreeCache(CuratorFramework client, String path) {
		this(client, path, true, false, Integer.MAX_VALUE, Executors.newSingleThreadExecutor(defaultThreadFactory),
				false, false, new DefaultTreeCacheSelector());
	}
	 ...
	 * @param executorService   Closeable ExecutorService to use for the TreeCache's
	 *                          background thread
	 ...
	 */
	TreeCache(CuratorFramework client, String path, boolean cacheData, boolean dataIsCompressed, int maxDepth,
			final ExecutorService executorService, boolean createParentNodes, boolean disableZkWatches,
	 ...
	 * There are no guarantees of accuracy; this is merely the most recent view of
	 * the data. If there is no node at this path, {@code null} is returned.
	 *
	 ...
	 * node at the given path, {@code null} is returned.
	 *
	 * @param fullPath full path to the node to check
