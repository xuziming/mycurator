...
/**
 * A re-entrant mutex that works across JVMs. Uses Zookeeper to hold the lock.
...
public class InterProcessMutex implements InterProcessLock, Revocable<InterProcessMutex> {
	 ...
	 * Each call to acquire must be balanced by a call to {@link #release()}
	 *
	 ...
	 * If the thread had made multiple calls to acquire, 
	 * the mutex will still be held when this method returns.
	 *
	...
	public void release() throws Exception {
		/*
		 * Note on concurrency: a given lockData instance can be only 
		 * acted on by a single thread so locking isn't necessary
		 */
