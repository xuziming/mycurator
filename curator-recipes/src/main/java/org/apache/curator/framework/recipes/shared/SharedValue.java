...
/**
 * Manages a shared value. All clients watching the same path will have the
 * up-to-date value (considering ZK's normal consistency guarantees).
 */
...
public class SharedValue implements Closeable, SharedValueReader {
	 ...
	 * @param seedValue the initial value for the value if/f the path has not yet
	 *                  been created
	 */
	...
	public VersionedValue<byte[]> getVersionedValue() {
		VersionedValue<byte[]> localCopy = currentValue.get();
		return new VersionedValue<byte[]>(localCopy.getVersion(),
				Arrays.copyOf(localCopy.getValue(), localCopy.getValue().length));
	}
	...
	/**
	 * Changes the shared value only if its value has not changed since this client
	 * last read it. If the value has changed, the value is not set and this
	 * client's view of the value is updated. i.e. if the value is not successful
	 * you can get the updated value by calling {@link #getValue()}.
	 *
	 * @deprecated use {@link #trySetValue(VersionedValue, byte[])} for stronger
	 *             atomicity guarantees. Even if this object's internal state is
	 *             up-to-date, the caller has no way to ensure that they've read the
	 ...
	 *         change was not successful, {@link #getValue()} will return the
	 *         updated value
	 * @throws Exception ZK errors, interruptions, etc.
	 */
	@Deprecated
	...
	/**
	 * Changes the shared value only if its value has not changed since the version
	 * specified by newValue. If the value has changed, the value is not set and
	 * this client's view of the value is updated. i.e. if the value is not
	 * successful you can get the updated value by calling {@link #getValue()}.
	 *
	 * @param newValue the new value to attempt
	 * @return true if the change attempt was successful, false if not. If the
	 *         change was not successful, {@link #getValue()} will return the
	...
	/**
	 * The shared value must be started before it can be used. Call {@link #close()}
	 * when you are finished with the shared value
	 *
	 * @throws Exception ZK errors, interruptions, etc.
	 */
	...
	private void notifyListeners() {
		...
		listeners.forEach(new Function<SharedValueListener, Void>() {
			@Override
