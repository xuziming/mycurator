...
public class TestingZooKeeperServer implements Closeable {
	 ...
	 * started again. If it is not running (in a LATENT or STOPPED state) then it
	 * will be restarted. If it is in a CLOSED state then an exception will be
	 * thrown.
	 *
	 * @throws Exception
	 */
		...
		new Thread(new Runnable() {
			public void run() {
				try {
					...
					logger.error(String.format("From testing server (random state: %s) for instance: %s",
